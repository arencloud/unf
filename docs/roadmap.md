...
- Phase 9.5a–9.5m now provide the canonical coalesced compiler and Causal Epoch
  ...
  encryption-generation-reconciler-test` and ADR 0172. The running agent
  plan/fact loop, TC consumption, verifier loading, and live encrypted packets
  remain within milestone 9.5. The Capability-Typed
  ...
  inert under `make encryption-linux-convergence-test`; ADR 0174. The running
  agent plan/fact loop and TC consumer remain next.
