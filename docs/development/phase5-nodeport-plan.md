...
Phase 5.5 must replace the deliberate `Local` fail-closed boundary with exact
node-local backend eligibility. It must preserve the external source tuple,
select only ready non-terminating backends placed on the receiving Node, return
an explicit no-local-backend outcome, survive endpoint placement/readiness
churn, define the health-check boundary, and prove direction-correct policy and
reverse translation for IPv4/IPv6 TCP/UDP without regressing `Cluster` mode.
