...
| 4.8 | OpenShift qualification | **In progress** | The Kind-qualified revision is published by three immutable public Quay digests. `make openshift-service-deploy` passed the guarded controller-first, `OnDelete`, five-Node ABI-v3→v4 handoff with 5/5 agents converged while kube-proxy remained available. `make openshift-service-test` is the remaining gate; it removes kube-proxy only after full convergence and owns the RHCOS/SELinux/CRI-O/lifecycle/recovery/operator proof |
