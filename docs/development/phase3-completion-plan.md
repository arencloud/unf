...
| 6.6 | Node-to-node networking | **In progress** | Distribution, native lifecycle, durable reconciliation, isolated dual-stack Kind operation, OpenShift bootstrap/remediation, live CNI 1.1 reconciliation, deferred offline DEL, clean-reboot recovery, and the committed self-cleaning CRI-O runtime-fault gate are Verified. Teardown and reprovision remain 6.6e2b |
...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADRs 0068–0072 verify the installation-time-only boundary, candidate package, remediation, guarded removal of all 25 temporary policies, live valid-attachment reconciliation, deferred offline DEL, same-IP reuse, controller-outage recovery, clean Node reboot, and deliberate CRI-O runtime-fault cleanup on cl02. Teardown and reprovision qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **In progress** | ADRs 0069–0072 and committed gates verify the package, remediation, GC, deferred-delete behavior, five-Node install, operator closure, zero-workaround traffic, same-IP reuse, controller-outage recovery, exact stale reconciliation, digest-pinned clean reboot, and deliberate CRI-O runtime-fault cleanup with exact final state. Teardown and reprovision remain |
...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Zero temporary policies, dual-stack canaries, kubelet probes, same-IP reuse, exact fixture cleanup, controller outage, agent LKG/new-epoch recovery, and 36-record CNI 1.1 reconciliation pass. Digest-pinned `15866c4` passed a boot-ID-changing reboot with early DEL recovery. `make openshift-primary-cni-runtime-fault-test` then verified CHECK, socket-offline CRI-O DEL, exact intent/lease retention, kernel link cleanup, recovery ADD ordering, dual-stack lease reuse, final 10/10/10/0 state, and 5/5 convergence. Remaining: exact teardown/no-CNI baseline and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
