...
| 6.6 | Node-to-node networking | **In progress** | Distribution, native lifecycle, durable reconciliation, isolated dual-stack Kind operation, OpenShift bootstrap/remediation, live CNI 1.1 reconciliation, deferred offline DEL, and the digest-pinned clean-reboot recovery gate are Verified. Deliberate CRI-O failure cleanup, teardown, and reprovision remain 6.6e2b |
...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADRs 0068–0072 verify the installation-time-only boundary, candidate package, remediation, guarded removal of all 25 temporary policies, live valid-attachment reconciliation, deferred offline DEL, terminal same-IP reuse, controller-outage recovery, and a clean Node reboot with exact self-recovery on cl02. CRI-O fault cleanup, teardown, and reprovision qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **In progress** | ADRs 0069–0072 and `make openshift-primary-cni-package-check` verify the package, remediation, GC, and deferred-delete behavior. The five-Node install, operator closure, zero-workaround traffic, terminal same-IP reuse, controller-outage recovery, exact stale reconciliation, and the digest-pinned ADR 0072 reboot pass. Ten socket-offline DELs drained before replacement ADDs and all five Nodes returned to exact cardinality without manual GC. CRI-O fault cleanup, teardown, and reprovision remain |
...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Corrected images run with all 25 temporary policies removed and five exactly converged agents. Dual-stack canaries, kubelet probes, terminal same-IP reuse, exact fixture cleanup, controller outage, server-node agent replacement/LKG restoration, new-epoch recovery, and 36-record CNI 1.1 reconciliation pass. Digest-pinned revision `15866c4` then passed a boot-ID-changing reboot: ten socket-offline DELs drained before replacement ADD, all Nodes returned to exact cardinality with empty queues, and BPF/routes/traffic/convergence recovered without manual GC. Remaining: deliberate CRI-O failure cleanup, exact teardown/no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
