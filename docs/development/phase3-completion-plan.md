...
| 6.6 | Node-to-node networking | **In progress** | Distribution, native lifecycle, durable reconciliation, isolated dual-stack Kind operation, and the OpenShift installation boundary are Verified; OpenShift bootstrap and RHCOS/SELinux live qualification remain 6.6e |
...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADR 0068 accepts an installation-time-only custom-CNI boundary and `make openshift-primary-cni-audit` verifies the fail-closed candidate classification. The audited cl02 is healthy but OVN-installed and cannot be force-converted. Digest-pinned bootstrap, exact MachineConfig/host ownership, teardown/recovery, and live qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **Planned** | Implement digest-pinned host-network bootstrap, exact MachineConfig/host ownership on `/var`, all-required-Node agent coverage, CRI-O ADD/CHECK/DEL, reboot/LKG recovery, dual-stack cross-worker and Service traffic, SELinux assertions, foreign-state refusal, and scoped no-CNI teardown; run only on a newly provisioned eligible cluster |
