...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Verified checkpoint: installer completion; completed pools; five Ready, Machine-linked Nodes; five converged agents; and external plus cross-worker IPv4/IPv6 traffic. ADR 0070 implements terminal-Pod retirement, revision-stable/NAT-aware replies, physical host-network peer lowering, and automatic revision-only Node-block refresh; the complete isolated primary-CNI Kind gate passes. Remaining: deploy immutable images to cl02, remove and audit all 24 temporary policies, then prove deliberate CRI-O failure cleanup, controller/agent/Node outage recovery, exact teardown, no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
