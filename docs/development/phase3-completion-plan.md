...
| 6.2 | CNI executable/configuration | **Verified** | `make cni-lifecycle-test` verifies bounded CNI 1.0/1.1 protocol behavior plus the atomic dual-stack ADD/CHECK/DEL lifecycle through the local transaction boundary; primary-CNI installation and cluster networking remain later isolated-cluster work |
| 6.2a | Local agent transaction state/API | **Verified** | `make cni-transaction-test` verifies schema-v2 bounded JSON, UID-0 Unix peer authentication, owner-only socket permissions, opt-in startup, deterministic attachment identity, atomic mode-0600 persistence, exact inspect/replay/conflict/transition behavior, durable preparing/ready/aborting/deleting restart recovery, and scoped shutdown/stale-socket handling; ADRs 0058 and 0060 |
| 6.2b | Atomic CNI resource transaction | **Verified** | `make cni-lifecycle-test` composes durable allocation, portable veth, and native routing as prepare → apply/readback → commit; proves exact ADD result/replay, CHECK plus prevResult and kernel drift, route-first DEL and repeated/missing-netns cleanup, preparing/aborting restart recovery, retained cleanup intent, foreign-route preservation, and lease release only after exact cleanup; ADR 0063 |
...
| 6.6 | Node-to-node networking | **In progress** | Distribution, native lifecycle, durable reconciliation, isolated dual-stack Kind operation, and the OpenShift boundary/bootstrap package are Verified. The first five-Node RHCOS primary-CNI install and dual-stack forwarding checkpoint passed; workaround elimination, outage/reboot recovery, teardown, and reprovision remain 6.6e2b |
...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADRs 0068–0069 verify the installation-time-only boundary, candidate audit, and digest-pinned activation package. A newly installed cl02 now completes OpenShift 4.22.10 with UNF primary networking and live CRI-O/dual-stack forwarding. The checkpoint required 24 annotated temporary policies; clean recovery, teardown, and reprovision qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **In progress** | ADR 0069 and `make openshift-primary-cni-package-check` verify the package. The five-Node Agent-based install, MachineConfigPool rollouts, Machine/Node linking, operator closure, agent convergence, and cross-worker plus external IPv4/IPv6 traffic now pass on OpenShift 4.22.10. The live checkpoint records four product gaps and 24 annotated bootstrap policies; clean outage/reboot recovery, teardown, and reprovision remain |
...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Verified checkpoint: installer completion; completed pools; five Ready, Machine-linked Nodes; five converged agents; and external plus cross-worker IPv4/IPv6 traffic. ADR 0070 implements terminal-Pod retirement, revision-stable/NAT-aware replies, physical host-network peer lowering, automatic revision-only Node-block refresh, one authoritative TC enforcement point, and linearly scaled exact Node exceptions. The first agent rollout exposed missing kubelet-probe coverage and was safely rolled back. Corrected immutable images from `be501c0` now run on cl02 with five exactly converged agents; the retained controller retry also corrected the host-network singleton to `Recreate`. Remaining: remove and audit the 24 original policies plus the exact DNS recovery policy, then prove deliberate CRI-O failure cleanup, controller/agent/Node outage recovery, exact teardown, no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
