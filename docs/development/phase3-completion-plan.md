...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Verified checkpoint: installer completion; completed master/worker MachineConfigPools; five Ready, Machine-linked Nodes; five converged agents with zero route errors; kube-proxy canary Service and external route; and bidirectional cross-worker IPv4/IPv6 Pod traffic. Remaining: remove 24 annotated bootstrap policies by fixing terminal-Pod IP reuse, reply-path/host-network identity, and controller-epoch Node-block recovery; then prove deliberate CRI-O failure cleanup, controller/agent/Node outage recovery, exact teardown, no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
