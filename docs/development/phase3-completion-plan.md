...
| 6.6 | Node-to-node networking | **In progress** | Distribution, native lifecycle, durable reconciliation, isolated dual-stack Kind operation, OpenShift bootstrap/remediation, live CNI 1.1 reconciliation, and local deferred offline DEL are Verified. The first five-Node RHCOS primary-CNI install and dual-stack forwarding checkpoint passed; ADR 0072 rollout and a second clean reboot, deliberate CRI-O failure cleanup, teardown, and reprovision remain 6.6e2b |
...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADRs 0068–0072 verify the installation-time-only boundary, candidate package, remediation, guarded removal of all 25 temporary policies, live valid-attachment reconciliation, and locally tested deferred offline DEL. Terminal same-IP reuse and controller-outage agent recovery pass on cl02; the ADR 0072 rollout and second clean Node reboot, CRI-O fault cleanup, teardown, and reprovision qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **In progress** | ADRs 0069–0072 and `make openshift-primary-cni-package-check` verify the package, remediation, GC, and deferred-delete behavior. The five-Node install, operator closure, zero-workaround traffic, terminal same-IP reuse, controller-outage agent recovery, immutable GC rollout, and exact five-Node stale reconciliation pass. A real reboot then exposed ten DEL calls before the agent socket and journal growth from 10 to 20; explicit GC restored equality. ADR 0072 rollout and second reboot, CRI-O fault cleanup, teardown, and reprovision remain |
...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Corrected `be501c0` images run with all 25 temporary policies removed and five exactly converged agents. Dual-stack canaries, kubelet probes, terminal same-IP reuse, exact fixture cleanup, controller outage, server-node agent replacement/LKG restoration, and new-epoch recovery pass. Revision `221168c` then reconciled 36 stale records across five Nodes. A boot-ID-changing reboot restored BPF/routes/traffic but early socket-offline DEL caused 10→20 journal growth; explicit GC restored 10/10/10. ADR 0072's `8f4165a` images rolled cleanly, but were held before reboot while the per-network lock was expanded across concurrent drain. Final `15866c4` images are published. Remaining: rollout and second reboot, deliberate CRI-O failure cleanup, exact teardown/no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
