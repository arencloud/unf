...
| 6.6 | Node-to-node networking | **In progress** | Controller-to-agent node-block distribution is Verified; cross-node dual-stack route lifecycle, failure recovery, observability, and coexistence qualification remain |
...
| 6.6b | Cross-node route intent and kernel lifecycle | **Planned** | Deterministic dual-stack remote-prefix IR, exact typed-kernel ownership, replay/readback/delete, conflict preservation, and atomic rollback |
| 6.6c | Cross-node reconciliation and recovery | **Planned** | Node/add/delete/restart convergence, stale-route retirement, revision acknowledgement, explanation/provenance, and failure injection |
| 6.6d | Isolated primary-CNI qualification | **Planned** | Install only on a disposable dual-stack Kind cluster, prove cross-worker lifecycle and rollback, then qualify RHCOS/SELinux behavior on cl02 |
