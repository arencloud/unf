...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADRs 0068–0070 verify the installation-time-only boundary, candidate package, remediation, and guarded removal of all 25 temporary policies. Terminal same-IP reuse and controller-outage agent recovery pass on cl02; clean Node reboot, CRI-O fault cleanup, teardown, and reprovision qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **In progress** | ADRs 0069–0070 and `make openshift-primary-cni-package-check` verify the package and remediation. The five-Node install, operator closure, zero-workaround traffic, terminal same-IP reuse, and controller-outage agent recovery pass. Probe hardening passed a 120-second live hold after a retained reboot retry exposed the generic one-second budget; clean reboot, CRI-O fault cleanup, teardown, and reprovision remain |
...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Corrected `be501c0` images run with all 25 temporary policies removed and five exactly converged agents. Dual-stack canaries, kubelet probes, terminal same-IP reuse, exact fixture cleanup, controller outage, server-node agent replacement/LKG restoration, and new-epoch recovery pass. Retained reboot attempts exposed and corrected stale-readiness fencing plus the controller's inherited one-second probe budget; the hardened live controller passed a 120-second zero-restart hold. Remaining: clean reboot rerun, deliberate CRI-O failure cleanup, exact teardown/no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
