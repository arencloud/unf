...
| 6.2 | CNI executable/configuration | **Verified** | `make cni-lifecycle-test` verifies bounded CNI 1.0/1.1 protocol behavior plus atomic dual-stack ADD/CHECK/DEL and CNI 1.1 valid-attachment GC through the local transaction boundary; ADRs 0063 and 0071 |
...
| 6.6 | Node-to-node networking | **In progress** | Distribution, native lifecycle, durable reconciliation, isolated dual-stack Kind operation, OpenShift bootstrap/remediation, and local CNI 1.1 GC are Verified. The first five-Node RHCOS primary-CNI install and dual-stack forwarding checkpoint passed; the GC image rollout and clean reboot, deliberate CRI-O failure cleanup, teardown, and reprovision remain 6.6e2b |
...
| 6.6e | OpenShift primary-CNI design and qualification | **In progress** | ADRs 0068–0071 verify the installation-time-only boundary, candidate package, remediation, guarded removal of all 25 temporary policies, and locally tested valid-attachment GC. Terminal same-IP reuse and controller-outage agent recovery pass on cl02; the GC image rollout and clean Node reboot, CRI-O fault cleanup, teardown, and reprovision qualification remain |
...
| 6.6e2 | OpenShift bootstrap, rollback, and live qualification | **In progress** | ADRs 0069–0071 and `make openshift-primary-cni-package-check` verify the package, remediation, and local GC behavior. The five-Node install, operator closure, zero-workaround traffic, terminal same-IP reuse, and controller-outage agent recovery pass. Probe hardening passed a 120-second live hold; the next reboot finding is 26 records with vanished sandboxes on one worker. Immutable GC rollout, reconciliation and clean reboot, CRI-O fault cleanup, teardown, and reprovision remain |
...
| 6.6e2b | OpenShift live primary-CNI lifecycle | **In progress** | Corrected `be501c0` images run with all 25 temporary policies removed and five exactly converged agents. Dual-stack canaries, kubelet probes, terminal same-IP reuse, exact fixture cleanup, controller outage, server-node agent replacement/LKG restoration, and new-epoch recovery pass. Retained reboot attempts corrected stale-readiness fencing and controller probes, then exposed 26 worker records whose links vanished when CRI-O could not deliver DEL. ADR 0071's standards-shaped GC passes local gates. Remaining: immutable rollout, stale-state reconciliation and clean reboot, deliberate CRI-O failure cleanup, exact teardown/no-CNI baseline, and reprovision. Evidence: `openshift-primary-cni-cl02-install.md` |
