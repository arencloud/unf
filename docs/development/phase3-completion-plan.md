...
| 6.5 | Routing and MTU | **In progress** | The provider-independent native route/neighbor and MTU plan is Verified; typed kernel apply/readback/delete, rollback, forwarding, and fragmentation qualification remain |
| 6.5a | Native route and MTU plan | **Verified** | `make cni-routing-test` proves zero-overhead native MTU derivation with the dual-stack minimum, durable MTU/readback drift rejection, `/32` and `/128` workload addressing, direction-scoped host endpoint plus container gateway/default route IR, permanent peer-MAC neighbor IR, and a private non-BGP route-protocol marker; ADR 0062 |
| 6.5b | Native route kernel lifecycle | **Planned** | Typed-netlink conflict preflight, apply, exact readback/delete, partial-failure rollback, and isolated namespace forwarding |
| 6.5c | MTU and fragmentation qualification | **Planned** | Dual-stack path-MTU behavior, underlay drift, boundary payloads, and exact rollback evidence |
