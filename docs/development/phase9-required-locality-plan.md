...
| Full authenticated controller and real reconciliation loops | Scoped helper selected (ADR 0449), superseding ADR 0448's lab SYS_ADMIN expansion. Detached-mount FD boundary implemented; supervisor, namespace observation/seeding, integration and exact restricted-runtime platform qualification remain open. No running agent privilege changed |
| Scoped helper detached-mount boundary | **Verified isolated slice** (ADR 0450): cl02 then identical Kind, SYS_ADMIN-only helper thread and three-capability client, denied client unshare, actual BPF pinning/reclamation, preserved journals and complete log review. Production process supervision and namespace observation/seeding remain separate; no fleet or Phase 9 promotion |
| Scoped helper owned-process supervision | **Implemented; live pending** (ADR 0451): sealed fixed executable, parent/child credentials, pidfd cancellation, one retained slot and actual child reaping. Privileged-bootstrap primitive only; final Pod delivery, parent-death/crash matrix, observations/seeding and integration remain open |
