...
| 8.8 | Reachability and advertisement providers | **Verified** | 8.8a–8.8e verify provider-neutral Diversity-Quorum Reachability, authenticated durable observers, the native lifecycle, causal-constrained dual-stack BGP, and dependency-aware BFD failure correlation. Platform qualification remains in 8.10–8.11 |
...
Milestone 8.10 now composes the complete Phase 8 runtime into one exact
kube-proxy-free dual-stack multi-Node Kind lifecycle. It must cover policy,
allocation, source steering/NAT, deterministic HA, temporal destination and
reachability evidence, recovery, causal operations, rollback, and exact
cleanup with immutable machine-readable evidence. OpenShift remains the
independent 8.11 platform gate.
