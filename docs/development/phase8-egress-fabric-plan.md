...
| 8.7 | FQDN and internet-access controls | **In progress** | 8.7a–8.7d verify temporal evidence, durable ownership, autonomous packet expiry, resolver-bound wildcard discovery, and a live multi-Node DNS lifecycle. Internet classification/fallback, explanation/simulation/operations, scale, and platform qualification remain |
...
Milestone 8.7e adds provider-neutral internet classification and explicit
fallback semantics without converting an unresolved name into a CIDR allow.
Classification revisions, provenance, exceptions, and failure behavior must be
independently replayable and must preserve the verified PLR, security-policy,
source-steering, gateway-NAT, and HA authority ordering. Later slices integrate
explanation, simulation, recovery, scale, and platform qualification.
