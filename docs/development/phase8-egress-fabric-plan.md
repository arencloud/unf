...
| 8.5 | Live distribution, source steering, and gateway NAT dataplane | **In progress** | Egress ABI v3 and exact 40-pin persistent ABI v14 provide destination-exact policy-first steering plus collision-safe dual-stack gateway SNAT/reverse state. Heterogeneous identity-namespaced gateway banks bind contract, lease, destination, local-primary gateway, digest, and proof witness; reverse-first `BPF_NOEXIST` pair insertion and a proof-salted full-cycle port permutation preserve existing flows. A digest-bound controller grant plus native route proof controls source activation, while Node-UID-bound `unf-egress0` ownership requires whole-host collision preflight and exact all-gateway readback quorum. `make egress-source-steering-test`, `make egress-path-activation-test`, `make egress-gateway-address-test`, and `make egress-gateway-nat-test`; ADRs 0126–0129. Release-barrier wiring, NAT events, and end-to-end traffic remain |
