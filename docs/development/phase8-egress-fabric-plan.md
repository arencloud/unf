...
| 8.5 | Live distribution, source steering, and gateway NAT dataplane | **In progress** | Egress ABI v2 and persistent ABI v13 provide destination-exact policy-first source steering. A digest-bound controller grant plus native route proof controls source activation. Exact Node-UID-bound `unf-egress0` ownership now installs dual-stack host addresses only after whole-host collision preflight and independently read-back kernel state; controller readiness requires current evidence from every selected gateway. Withdrawals quarantine ownership and allocator leases until an explicit future source-fence/reachability release barrier. `make egress-source-steering-test`, `make egress-path-activation-test`, and `make egress-gateway-address-test`; ADRs 0126–0128. Collision-safe SNAT/reverse state, release-barrier wiring, events, and end-to-end traffic remain |
...
Milestone 8.5 next consumes the verified source activation and gateway address
ownership prerequisites in source/gateway TC stages attached after the existing
egress policy decision. It adds proof-keyed, collision-safe dual-stack TCP/UDP
SNAT/reverse state and kernel packet gates, then closes withdrawal with explicit
source-fence and reachability evidence before address release. Address ownership
alone does not advertise a route, translate a packet, or make a platform claim.
