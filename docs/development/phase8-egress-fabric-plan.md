...
| 8.7 | FQDN and internet-access controls | **Verified** | 8.7a–8.7f verify temporal DNS evidence, durable ownership, autonomous packet expiry, resolver-bound wildcard discovery, live multi-Node DNS and classifier lifecycles, and provider-neutral Authority-Carved Internet classification with authenticated durable ingestion and explicit bounded fallback. Later reachability, operations, and platform milestones remain independently gated |
...
| 8.8 | Reachability and advertisement providers | **Verified** | 8.8a–8.8e verify provider-neutral Diversity-Quorum Reachability, authenticated durable observers, the native lifecycle, causal-constrained dual-stack BGP, and dependency-aware BFD failure correlation. Kind and OpenShift composition are independently verified in 8.10 and 8.11 |
...
| 8.9 | Operations, simulation, upgrade, and recovery | **Verified** | 8.9a–8.9c verify loss-explicit history, evidence-complete non-authoritative counterfactuals, a complete additive egress compatibility vector, cross-checkpoint restart ordering, adjacent control-plane migration/rollback refusal, provider/agent replay, and exact current-ABI cleanup; ADRs 0153–0155 |
...
| 8.9b | Causal explanation, failover history, and counterfactual simulation | **Verified** | Evidence-complete explain/simulate joins policy, intent, allocation, gateway, contract, reachability, source activation, Chronicle, transport, and HA state while distinguishing authoritative, observed, derived, expired, unavailable, and loss-affected evidence. Terminal promotions enter a bounded hash-chained checkpoint-v6 ledger with an eviction anchor; operations carry no grants and never infer private NAT state. Fixed-cardinality status and dedicated CLI/API surfaces pass `make egress-operations-causal-test`; ADR 0154 |
| 8.9c | Upgrade, recovery, compatibility, and exact cleanup | **Verified** | The Causal Egress Recovery Vector exposes distribution, host, HA, map, and event schemas additively; nonzero drift rejects before BPF access while absent adjacent fields remain independently payload-fenced. Restart permits only exact state or desired-ahead reconciliation, v5 migrates to v6, evidence-losing rollback/future schemas fail closed, and exact current-v15 cleanup refuses foreign pins and preserves adjacent/sibling state. Controller/provider/agent replay and strict Clippy pass `make egress-upgrade-recovery-test`; ADR 0155 |
| 8.10 | Kube-proxy-free Kind qualification | **Verified** | Runtime and qualifier `2f404ed` passed the uninterrupted 1,013-second three-Node dual-stack Kubernetes v1.35.0 gate. Six digest-bound components cover watched policy/allocation/steering/NAT, measured HA with zero-churn warm-standby rejoin, FQDN and Internet authority, DQR/native reachability, causal operations, restart recovery, proof-authorized interface-absence release, exact cleanup, and no-CNI rollback. Aggregate SHA-256 `c364a99a05f1bd9a1b0416bf3305feabc3de58d25119bfc20b0fc66bb7efbc88`; ADR 0156 |
| 8.11 | OpenShift qualification | **Verified** | Runtime `2f404ed` and qualifier `baf2bb0` passed the 411-second five-Node dual-stack OpenShift 4.22.10/Kubernetes 1.35.6 cl02 gate. Immutable images proved RHCOS/SELinux/CRI-O operation without kube-proxy, external IPv4/IPv6 source observation, four-address exclusive ownership across three gateways, graceful HA and zero-churn standby recovery, controller/agent replacement, operations, exact cleanup, five-agent convergence, and unchanged `network` unhealthy baseline. Evidence SHA-256 `a2f8cb2279a3e1417ad1533575b644487e64cbfd1d8afafe351e99fad7e126d3`; ADR 0157 |
...
Phase 8 does not silently claim production-scale BGP/ECMP/BFD availability,
EVPN, BFD authentication or IPv6 BFD transport, cloud-provider adapters,
cross-cluster egress, overlapping-CIDR translation, WireGuard, application
identity from DNS, L7 proxying, Gateway API, SCTP egress NAT, fragments,
generic NAT `RELATED`, arbitrary ICMP error translation, production HA, availability, or scale.
Those require independent architecture and gates.

## Closure evidence

Milestone 8.11 took the exact committed Phase 8 runtime through an independent
digest-pinned five-Node dual-stack OpenShift cl02 gate. Runtime `2f404ed` and
qualifier `baf2bb0` passed in 411 seconds on OpenShift 4.22.10/Kubernetes
1.35.6. The gate verified RHCOS/SELinux/CRI-O operation, cross-worker source
steering and NAT, externally observed IPv4/IPv6 egress addresses,
multi-address HA with zero-churn warm-standby restoration, controller and agent
recovery, causal operations, exact cleanup, five-agent convergence, and no new
unhealthy ClusterOperator relative to the captured `network` baseline. The
schema-v1 evidence is `.artifacts/phase8-egress-complete-openshift.json`,
SHA-256 `a2f8cb2279a3e1417ad1533575b644487e64cbfd1d8afafe351e99fad7e126d3`;
ADR 0157. Together with the independent 8.10 Kind result, this closes Phase 8.
