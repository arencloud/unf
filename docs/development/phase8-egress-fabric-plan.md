...
| 8.5 | Live distribution, source steering, and gateway NAT dataplane | **In progress** | Egress ABI v2 and persistent ABI v13 provide destination-exact policy-first source steering. A digest-bound controller grant now requires current application evidence from every selected gateway, while the source independently reads back the exact native dual-stack route, Node UID, transport, interface index, MTU, and stable route revision before atomically entering `Active`. Withdrawal, proof loss, sync failure, and restart return through destination-preserving fences. `make egress-source-steering-test` and `make egress-path-activation-test`; ADRs 0126–0127. Gateway address ownership, collision-safe SNAT/reverse state, events, and end-to-end traffic remain |
...
Milestone 8.5 next consumes the verified readiness/path activation prerequisite in
source/gateway TC stages attached after the existing egress policy decision,
then adds collision-safe dual-stack TCP/UDP SNAT/reverse state and kernel packet
gates. Current application acknowledgement changes no host routing, applied
address ownership, gateway NAT maps, packet behavior, or platform claim.
