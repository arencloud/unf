...
| 8.5 | Live distribution, source steering, and gateway NAT dataplane | **In progress** | The latest verified slice moves egress maps to ABI v2 and the durable boundary to exact 33-pin ABI v13. Intent-and-bank-prefixed IPv4/IPv6 destination LPM state prevents identity-only overcapture; policy denial precedes exact-match original-tuple selection and unchanged-tuple direct-neighbor handoff, while fences/corruption/skew/unsupported tunnel/MTU fail closed. Real-kernel dual-stack packets pass `make egress-source-steering-test`; ADR 0126. Live path acquisition and readiness-driven activation, gateway address ownership, collision-safe SNAT/reverse state, events, and end-to-end traffic remain |
...
Milestone 8.5 next consumes the verified bilateral-readiness prerequisite in
