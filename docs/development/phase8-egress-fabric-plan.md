...
| 8.7 | FQDN and internet-access controls | **In progress** | 8.7a–8.7e verify temporal DNS evidence, durable ownership, autonomous packet expiry, resolver-bound wildcard discovery, a live multi-Node DNS lifecycle, and provider-neutral Authority-Carved Internet classification with explicit bounded fallback. Durable classifier ingestion/live lifecycle, explanation/simulation/operations, scale, and platform qualification remain |
...
Milestone 8.7f adds authenticated, durable classifier ingestion and a live
dual-stack classification lifecycle. It must transactionally retain the exact
last-known-good classification, refresh source and gateway banks before
deadline, prove current/exception/private/unknown/loss/recovery behavior with
real traffic, and preserve the verified PLR, security-policy, source-steering,
gateway-NAT, and HA authority ordering. Later slices integrate explanation,
simulation, scale, and platform qualification.
