...
| 9.5 | Intent-Coalesced Cryptographic Fast Path | **In progress** | Phase 9.5a adds a canonical bounded compiler, explicit identity decisions, exact Node/epoch transport coalescing, fixed-width ABI, and the Causal Epoch Lease under `make encryption-fast-path-contract-test`; ADR 0162. Phase 9.5b adds the Causal Commit Vector and durable transaction contract binding policy/Service/egress revisions, exact kernel commitments, epoch frontier, inactive-bank readback, one pointer flip, total crash recovery, and positive rollback under `make encryption-fast-path-transaction-test`; ADR 0163. Phase 9.5c adds a separately versioned four-map Aya ABI island with exact all-or-none ownership and quarantine-first recovery under `make encryption-map-persistence-test`; ADR 0164. Phase 9.5d adds the Proof-Carrying Aya Map Mirror, delta-minimal inactive staging, exact byte readback, atomic publication, and wired total restart recovery under `make encryption-map-transaction-test`; ADR 0165. Phase 9.5e adds the collision-free Cooperative Route-Mark Lease under `make encryption-route-mark-contract-test`; ADR 0166. Controller distribution, TC marking, policy routes, verifier, and live traffic remain before Verified |
...
Complete milestone 9.5 by feeding compiled generations into the 9.5d Aya
transaction boundary; atomically install the 9.5e masked policy-rule/table
lease before publication; invoke bounded lookup after policy plus Service/egress
selection in TC; and pass verifier, restart, rotation, revocation,
IPv4/IPv6, Service, egress, and live-kernel traffic gates. The compiler/ABI,
transaction contract, isolated persistence foundation, and proof-carrying map
mirror plus cooperative mark contract now pass their focused gates, but no
workload packet-path activation is claimed yet. Live two-ended encrypted
challenge proof remains milestone 9.6.
