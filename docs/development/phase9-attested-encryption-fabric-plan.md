...
| 9.6 | Bidirectional live path proof | **In progress** | Phase 9.6a adds the Causal Duplex Path Quorum over exact contract, kernel, counter, transcript, identity, family, and expiry evidence. Phase 9.6b adds the generation-fenced Pull-Synchronized Duplex Proof Exchange. Phase 9.6c adds an Evidence-Carrying Activation Choke Point: exact current receipts cover every Required decision once, bind the selected kernel transport, and remain consuming authority through Linux convergence to the final inactive-bank boundary; legacy activation denies Required state. `make encryption-path-activation-test`; ADRs 0194–0196. Agent-side encrypted nonce execution remains next |
