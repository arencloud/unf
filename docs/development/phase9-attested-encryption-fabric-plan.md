...
Complete milestone 9.5 by publishing Node-local keys through the 9.5u ledger,
producing authoritative catalog cuts, and feeding the Node-local compiler and
exact exchange/activation/restart path;
...
compile runtime inbox, fleet-synchronous catalog, and public-key transparency
cut now pass their focused gates, but no workload packet-path activation is
claimed yet.
