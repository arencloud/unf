...
Complete milestone 9.5 by feeding authenticated, durable inputs into the 9.5p
Node-local compiler and exact exchange/activation/restart path;
...
proof-rehydrating restart activation, and snapshot-first local plan compilation
now pass their focused gates, but no
