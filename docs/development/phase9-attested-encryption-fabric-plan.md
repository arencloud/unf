...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0267 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and bounded recovery/evidence boundaries. `3ffa0a6` passed preserved-state deployment but its Required transition exposed response bodies escaping the one-materializer lifetime and OOM-killed the controller. End-to-End Authority Delivery Lease keeps the bounded FIFO permits through complete body delivery. Exact successor `3026662` passed fresh Kind and is published by immutable public digest. Preserved-state deployment and the complete cl02 gate remain |
