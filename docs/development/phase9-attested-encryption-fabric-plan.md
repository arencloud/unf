...
| 9.8 | Kube-proxy-free Kind qualification | **Verified** | Exact runtime and qualifier `0590d9b` passed the complete fresh three-Node dual-stack Kubernetes v1.35.0 gate with ADR 0251. Default-Required and explicit-selective PodIP/Service traffic, 401 WireGuard frames with zero Required plaintext, eight-of-eight fail-closed Required probes with eight-of-eight Native successes, natural rotation, agent/controller replacement, 354 loss-free operation records, performance, Phase 8 egress coexistence, exact cleanup, and no-CNI rollback passed. Evidence JSON SHA-256 is `0023b63…b5b5`; packet-capture SHA-256 is `fcbaf3c…7caa` |
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0252 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and bounded recovery/evidence boundaries. Runtime `0590d9b` recovered the preserved consecutive tombstone chain, but the Required switch exposed a cluster-wide Cartesian scan across policies that could not select a pair. The Policy-Relevance Quotient caches enriched endpoints, evaluates only exact applicable partitions, preserves full evaluator semantics, and fails closed before an explicit class-work bound. Fresh Kind publication, preserved-state cl02 recovery, and the complete platform gate remain |
