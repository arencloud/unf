...
| 9.8 | Kube-proxy-free Kind qualification | **Verified** | Runtime and qualifier `6d29ac3` passed the fresh three-Node dual-stack Kubernetes v1.35.0 gate after all OpenShift-derived safety fixes. Default-required and explicit-selective PodIP/Service traffic, 409 WireGuard frames with zero Required plaintext, eight-of-eight fail-closed Required probes with eight-of-eight Native successes, exact link repair, agent replacement, natural rotation, controller replacement with demand-driven activation-history reconstruction, loss-free operations, performance, Phase 8 egress coexistence, exact cleanup, and no-CNI rollback passed. Evidence JSON SHA-256 is `c432111d…fde`; packet-capture SHA-256 is `d5ace897…f73`. ADRs 0208–0219 |
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0221 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and recovery boundaries. Runtime `6d29ac3` passed Kind, but the first cl02 transition stopped before qualification when unmanaged TCP lost foreign host mark metadata after fleet activation. Identity-Scoped Packet-Mark Ownership fixes that boundary. A subsequent clean Kind run exposed a distinct restart/rotation liveness fault: a partially torn-down retiring interface could not satisfy pristine-state deletion and permanently occupied the two-epoch key window. Proof-Carrying Monotonic Retirement now permits only missing plan-owned state and refuses every mutation/addition. Its privileged kernel regression passes; a corrected immutable runtime must repeat fresh Kind, recover cl02 to its known baseline, and pass the full five-Node gate |
