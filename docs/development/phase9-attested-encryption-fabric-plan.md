...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0251 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and bounded recovery/evidence boundaries. A cl02 reboot proved two consecutive admitted generations can be tombstoned before a fresh successor arrives. The Compact Causal Tombstone Chain retains verified transaction edges back to the physical map while remaining zero-authority. Exact runtime `0590d9b` now has fresh complete Kind evidence and immutable public images; preserved-state cl02 recovery and the complete platform gate remain |
