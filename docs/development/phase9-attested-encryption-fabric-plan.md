...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0237 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration, recovery, non-perturbing evidence, causal-readiness, monotonic-platform-health, activation-supersession, and immutable requalification boundaries. Live cl02 work exposed observer churn, a simultaneous five-Node expired Active-plus-Prepared key deadlock, and a valid predecessor activation report fencing its newer prepared generation. Expired Authority Recovery renews unusable authority without plaintext fallback; Crash-Residue Cleanup Closure makes rollback interruption-total; Conflict-Acknowledged Activation Supersession retires only a controller-confirmed stale outbox entry. Exact revised runtime `e1dbb94` passed fresh full Kind; reboot recovery proof and the complete cl02 gate remain |
