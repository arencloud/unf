...
| 9.6 | Bidirectional live path proof | **In progress** | Phase 9.6a adds the Causal Duplex Path Quorum: one short-lived nonce binds the exact contract/witness, Node UIDs, epoch, families, before/after kernel configuration and observation digests, exact peer endpoint/key/AllowedIPs/routes/fwmark/MTU, positive bidirectional counter deltas, and a common challenge transcript. Only distinct authenticated source and destination proofs issue a receipt; one-sided, expired, roaming, replayed, stalled, or mutated evidence denies closed under `make encryption-path-proof-test`; ADR 0194. Runtime collection and consuming activation remain 9.6b |
