...
| 9.8 | Kube-proxy-free Kind qualification | **In progress** | Phase 9.8a adds Content-Addressed Membership Resume: a domain-separated exact Node-name/UID coordinate ignores unrelated topology churn, fences replacement, and lets a restarted controller reconstruct already mutually-attested public authority without reopening an impossible witness round; focused tests and strict Clippy pass; ADR 0208. One exact committed runtime must now pass the complete live gate |
