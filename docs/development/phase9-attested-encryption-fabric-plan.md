...
Complete milestone 9.5 by producing authoritative 9.5t catalog cuts, feeding
the Node-local compiler and exact exchange/activation/restart path;
...
compile runtime inbox, and fleet-synchronous catalog now pass their focused
gates, but no workload packet-path activation is claimed yet.
