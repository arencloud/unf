...
| 9.7 | Operations, upgrade, recovery, and performance | **In progress** | Phases 9.7a–c provide the Causal Evidence Watermark, poll-stable runtime export, and a Restart-Continuous Evidence Chain: exact-name bounded persistence validates the hash chain/counters before restore and resumes all 54 Prometheus cells. `make encryption-operations-recovery-test`; ADRs 0200–0202. Authenticated agent activation, explanation/simulation, adjacent upgrade/rollback, outage/replacement/cleanup, and committed performance evidence remain |
...
Continue milestone 9.7 with authenticated, retry-stable agent-side activation
acknowledgement. Then add exact explanation and non-authoritative simulation,
adjacent upgrade/rollback,
