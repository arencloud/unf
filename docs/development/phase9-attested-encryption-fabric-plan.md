...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0229 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and recovery boundaries. Exact runtime `32b5501` passed fresh full Kind after zero-transport closure and activation-before-successor recovery. Anonymous inspection proved the release-bound controller, agent, and test-tool digests are public Linux/amd64 manifests. The complete cl02 gate remains |
...
Resume the five-Node cl02 qualification with only the ADR 0229 digest-pinned
tuple. The OpenShift
