...
Complete milestone 9.5 by wiring the 9.5l proof ladder to the agent's live
Node-local key authority, WireGuard configuration/readback, controller fact
submission, policy routes, and Aya adapter;
...
recovery, authenticated complete-cut fact reconciliation, and capability-typed
local proof-ordering contracts now pass their focused gates, but no workload
