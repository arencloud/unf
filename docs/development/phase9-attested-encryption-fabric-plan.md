...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0222 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and recovery boundaries. Runtime `6d29ac3` passed Kind, but cl02 exposed unmanaged host-mark corruption. Fresh qualification then exposed two fail-closed liveness boundaries after the link-fault stage: pristine-only retirement blocked an owned partial epoch, and route-permit issuance ran before exact-plan route self-healing. Proof-Carrying Monotonic Retirement and Repair-Before-Permit Activation now make bounded teardown and activation recoverable without accepting partial authority. Focused real-kernel gates pass; a corrected immutable runtime must repeat fresh Kind, recover cl02, and pass the full five-Node gate |
