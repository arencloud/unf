...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0219 and `make encryption-phase9-openshift-gate-test` define a digest-pinned, explicitly acknowledged Native-to-Required migration, a Causal Pre-Attachment Fleet Barrier, a Monotonic Fleet Key Epoch Floor, reciprocal active-generation proof participation, forward cursor recovery, exact kernel self-healing, and demand-driven reconstruction of missing activation history for safe mixed-version, interrupted-preparation, and controller recovery. Runtime `6d29ac3` passed the complete fresh Kind gate. The five-Node dual-stack cl02 RHCOS/SELinux/CRI-O gate now remains, covering cross-worker PodIP/Service ciphertext, fail-closed selective traffic, rotation/recovery, loss-free operations, exact cleanup, convergence, kube-proxy absence, and ClusterOperator comparison |
