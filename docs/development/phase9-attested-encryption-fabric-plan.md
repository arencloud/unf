...
| 9.7 | Operations, upgrade, recovery, and performance | **In progress** | Phases 9.7a–d provide the Causal Evidence Watermark, poll-stable runtime export, restart-continuous persistence, and an Acknowledged Activation Outbox. Post-map reports bind current Node/generation/state/path truth, queue across controller outage, deduplicate through durable cursors, and never become authority. `make encryption-activation-report-test`; ADRs 0200–0203. Explanation/simulation, adjacent upgrade/rollback, outage/replacement/cleanup, and committed performance evidence remain |
...
Continue milestone 9.7 with exact causal explanation and non-authoritative
simulation. Then add adjacent upgrade/rollback, outage/replacement/cleanup, and
committed native-versus-encrypted performance evidence as independent verified
slices.
