...
Complete milestone 9.5 by feeding the 9.5i frontier from live authoritative
facts, persisting its cut/receipt state, and implementing the local orchestrator
that configures/read-backs WireGuard and routes before issuing the 9.5h latch;
...
live-kernel traffic gates. The compiler/ABI,
...
delivery, tri-plane activation, and cluster-complete publication contracts now
pass their focused gates, but no workload packet-path activation is claimed
yet. Live two-ended encrypted challenge proof remains milestone 9.6.
