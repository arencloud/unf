...
| 9.7 | Operations, upgrade, recovery, and performance | **In progress** | Phases 9.7a–e provide the Causal Evidence Watermark, poll-stable export, restart-continuous persistence, acknowledged post-map activation, and Minimum Causal Cut Explanation. Policy-first explain/simulate joins exact path evidence, reports the earliest missing prerequisite, marks loss, and proves counterfactual reads cannot mutate authority or evidence. `make encryption-operations-query-test`; ADRs 0200–0204. Adjacent upgrade/rollback, outage/replacement/cleanup, and committed performance evidence remain |
...
Continue milestone 9.7 with adjacent upgrade/rollback compatibility. Then add
