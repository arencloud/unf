...
Complete milestone 9.5 by applying the CCV transaction to the pinned Aya
decision, transport, configuration, and CEL maps now established by 9.5c;
replace the quiescent-only recovery fence with exact durable inactive/active
bank recovery; invoke bounded lookup after policy plus Service/egress selection
in TC; preserve unrelated skb mark bits while selecting the exact WireGuard
policy route; and pass verifier, restart, rotation, revocation, IPv4/IPv6,
Service, egress, and live-kernel traffic gates. The 9.5a compiler/ABI, 9.5b
transaction contract, and 9.5c isolated persistence foundation pass their
focused gates, but no workload packet-path activation is claimed yet. Live
two-ended encrypted challenge proof remains milestone 9.6.
