...
| 9.7 | Operations, upgrade, recovery, and performance | **In progress** | Phase 9.7a adds the Causal Evidence Watermark; 9.7b wires poll-stable assignment/local-proof/remote-quorum transitions to public status/history and exactly 54 preallocated Prometheus series without topology labels. `make encryption-operations-runtime-test`; ADRs 0200–0201. Authenticated durable activation, explanation/simulation, adjacent upgrade/rollback, outage/replacement/cleanup, and committed performance evidence remain |
...
Continue milestone 9.7 with authenticated agent-side activation acknowledgement
and durable loss-explicit controller recovery. Then add exact explanation and
non-authoritative simulation, adjacent upgrade/rollback,
