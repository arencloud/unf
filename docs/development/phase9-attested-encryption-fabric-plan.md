...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0215 and `make encryption-phase9-openshift-gate-test` define a digest-pinned, explicitly acknowledged Native-to-Required migration, a Causal Pre-Attachment Fleet Barrier, and a Monotonic Fleet Key Epoch Floor for safe mixed-version and interrupted-preparation recovery. The five-Node dual-stack cl02 RHCOS/SELinux/CRI-O gate covers cross-worker PodIP/Service ciphertext, fail-closed selective traffic, rotation/recovery, loss-free operations, exact cleanup, convergence, kube-proxy absence, and ClusterOperator comparison. The epoch-floor runtime must repeat Kind before live OpenShift execution |
