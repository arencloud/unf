...
| 9.5 | Intent-Coalesced Cryptographic Fast Path | **In progress** | Phase 9.5a adds the canonical compiler and Causal Epoch Lease. Phase 9.5b adds the Causal Commit Vector and total recovery contract. Phase 9.5c–9.5f add the isolated proof-carrying Aya ABI, Cooperative Route-Mark Lease, and Route-Before-Authority. Phase 9.5g adds the Node-Sealed Generation Capsule: TokenReview-authenticated, nonce/Node-UID/incarnation/predecessor-bound exact-successor delivery with mode-0600 agent persistence and no transferable route permit under `make encryption-generation-distribution-test`; ADR 0168. Prepared-generation production, local activation orchestration, TC consumption/verifier, and live encrypted traffic remain before Verified |
...
Complete milestone 9.5 by producing prepared per-Node generations from live
authority, reconstructing local WireGuard/route evidence and joining the
non-transferable 9.5f permit to the durably admitted 9.5g generation at the
agent transaction boundary; invoke bounded lookup after policy plus
Service/egress selection in TC; and pass
verifier, restart, rotation, revocation,
...
mirror, cooperative mark, policy-route activation, and authenticated causal
delivery contracts now pass their focused gates, but no workload packet-path
activation is claimed yet. Live two-ended encrypted
