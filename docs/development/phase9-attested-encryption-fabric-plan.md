...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0228 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and recovery boundaries. Runtime `ec76916` passed fresh full Kind; cl02 staging exposed the zero-transport edge fixed by ADR 0227. The first new Kind run then exposed restored-cut advancement before one admitted Node completed path proof. Activation-Before-Recovery-Successor retains the reconstructible cut until its durable fleet activation cursor closes; controller regression, all 107 controller tests, strict Clippy, and static gates pass. A new exact full Kind qualification, immutable publication, and the complete cl02 gate remain |
...
Requalify the ADR 0228 runtime on fresh dual-stack Kind, publish its exact
registry manifests, and resume the five-Node cl02 qualification. The OpenShift
