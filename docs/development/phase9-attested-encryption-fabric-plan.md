...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0220 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and recovery boundaries. Runtime `6d29ac3` passed Kind, but the first cl02 transition stopped before qualification when unmanaged TCP lost foreign host mark metadata after fleet activation. Identity-Scoped Packet-Mark Ownership now preserves an identity-incomplete packet's complete mark and passes a real kernel TC regression. A corrected immutable runtime must repeat fresh Kind, recover cl02 to its known baseline, and pass the full five-Node dual-stack RHCOS/SELinux/CRI-O, ciphertext, selective fail-closed, rotation/recovery, loss-free operations, exact cleanup, convergence, kube-proxy-absence, and ClusterOperator gate |
