...
| 9.5 | Intent-Coalesced Cryptographic Fast Path | **In progress** | Phase 9.5a adds the canonical compiler and Causal Epoch Lease. Phase 9.5b adds the Causal Commit Vector and total recovery contract. Phase 9.5c–9.5f add the isolated proof-carrying Aya ABI, Cooperative Route-Mark Lease, and Route-Before-Authority. Phase 9.5g adds authenticated exact-successor capsules. Phase 9.5h adds the non-serializable Tri-Plane Causal Activation Latch, requiring the capsule, renewed local route permit, and exact applied/quarantined Aya transaction to agree before publication or restart attachment under `make encryption-activation-latch-test`; ADRs 0168–0169. Prepared-generation production, live local orchestration, TC consumption/verifier, and encrypted traffic remain before Verified |
...
authority and implementing the local orchestrator that configures/read-backs
WireGuard and routes before issuing the 9.5h latch; invoke bounded lookup after policy plus
...
delivery, and tri-plane activation contracts now pass their focused gates, but no workload packet-path
