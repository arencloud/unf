...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0233 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration, recovery, and non-perturbing evidence boundaries. Live cl02 work exposed observer churn and a simultaneous five-Node expired Active-plus-Prepared key deadlock. Expired Authority Recovery renews the unusable authority without plaintext fallback, and Crash-Residue Cleanup Closure makes its rollback interruption-total. Exact runtime `7296f06` passed fresh full Kind and is bound to public immutable images/evidence by ADR 0233. Reboot recovery proof and the complete cl02 gate remain |
...
qualification with the ADR 0230 non-perturbing witness. The OpenShift gate must preserve
