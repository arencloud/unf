...
| 9.8 | Kube-proxy-free Kind qualification | **Verified** | Exact runtime and qualifier `e1dbb94` passed the complete fresh three-Node dual-stack Kubernetes v1.35.0 gate with ADR 0237. Default-Required and explicit-selective PodIP/Service traffic, 340 WireGuard frames with zero Required plaintext, eight-of-eight fail-closed Required probes with eight-of-eight Native successes, natural rotation, agent/controller replacement, 295 loss-free operation records, performance, Phase 8 egress coexistence, exact cleanup, and no-CNI rollback passed. Evidence JSON SHA-256 is `870e920…05f5`; packet-capture SHA-256 is `c02654e…c94` |
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0239 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration, recovery, non-perturbing evidence, causal-readiness, monotonic-platform-health, activation-supersession, immutable requalification, and fully bounded witness boundaries. Live cl02 work exposed observer churn, a simultaneous five-Node expired Active-plus-Prepared key deadlock, a valid predecessor activation report fencing its successor, and unbounded API samples. Expired Authority Recovery renews unusable authority without plaintext fallback; Conflict-Acknowledged Activation Supersession retires only a controller-confirmed stale outbox; Node-local loopback and whole-gate request deadlines make qualification finite without cached evidence. Exact revised runtime `e1dbb94` passed fresh full Kind; the complete cl02 gate remains |
...
Deploy the ADR 0237 immutable activation-supersession tuple and prove that the
simultaneously rebooted five-Node cl02 fleet replaces its stale Native
generation. Rerun the complete
qualification with the ADR 0230 non-perturbing witness, ADR 0234 causal
readiness join, ADR 0235 monotonic platform-health delta, and ADR 0238 bounded
Node-local control witness plus ADR 0239 whole-gate API deadline. The OpenShift gate
must preserve RHCOS/SELinux/CRI-O facts, cross-worker encrypted IPv4/IPv6 direct
and Service traffic, rotation/recovery, exact cleanup, agent convergence,
kube-proxy absence, and the before/after ClusterOperator state.
