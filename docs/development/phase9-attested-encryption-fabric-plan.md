...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0243 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and fully bounded recovery/evidence boundaries. Admitted Predecessor Settlement completes a controller-admitted pending generation through normal activation before compiling its successor. Exact runtime `180ae2f` passed fresh full Kind and is bound to immutable public images; the complete cl02 gate remains |
...
Deploy the ADR 0243 immutable admitted-predecessor tuple to the simultaneously
