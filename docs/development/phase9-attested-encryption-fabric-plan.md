...
| 9.8 | Kube-proxy-free Kind qualification | **Verified** | Exact runtime and qualifier `73057c1` passed the complete fresh three-Node dual-stack Kubernetes v1.35.0 gate with ADR 0261. Default-Required and explicit-selective PodIP/Service traffic, 336 WireGuard frames with zero Required plaintext, eight-of-eight fail-closed Required probes with eight-of-eight Native successes, natural rotation, agent/controller replacement, 295 loss-free operation records, performance, Phase 8 egress coexistence, exact cleanup, and no-CNI rollback passed. Evidence JSON SHA-256 is `b6845fd…42cb`; packet-capture SHA-256 is `66341a4…972d` |
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0262 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and bounded recovery/evidence boundaries. `73057c1` proved five-agent cold admission in 81 seconds with zero controller restarts and 136 MiB RSS, then failed serial replacement because encryption identity preflight treated intentional admission `503` as fatal. Causal Startup Admission Retry now shares one bounded pre-BPF predicate across both bootstrap reads. Fresh Kind, immutable public images, preserved-state deployment, and the complete cl02 platform gate remain |
...
Validate ADR 0262's Causal Startup Admission Retry, run a completely fresh Kind
lifecycle, and publish only the exact digest-pinned successor. Deploy it to
