...
| 9.5 | Intent-Coalesced Cryptographic Fast Path | **In progress** | Phase 9.5a–9.5ac connect the canonical compiler, proof-carrying maps/routes, authenticated complete-cut distribution, local Linux/key proof, fleet production, policy/placement truth, and exact quiescent generations. Phase 9.5ad adds Proof-Carrying Deferred Encryption: a verifier-isolated, policy-first final-tuple TC consumer with dual-stack late binding, Causal Epoch Leases, cooperative marking, MTU/revision proof, and Required fail-closed behavior. Phase 9.5ae adds Flow-Adaptive Secure DSR, preserving Native direct return while morphing only Required flows to reversible backend-addressed NAT, plus egress precedence and bounded rotation/revocation packet proof under `make encryption-composition-test`; ADRs 0162–0192. Live WireGuard ciphertext remains before Verified |
