...
| 9.7 | Operations, upgrade, recovery, and performance | **In progress** | Phases 9.7a–g provide causal operations, minimum-cut diagnosis, adjacent compatibility, and the Survivable Authority Envelope. Outage retains only independently recoverable authority; reachable Node identity fences stale plan/generation/recovery/key state before BPF access; exact current cleanup removes the six-pin encryption island first and preserves foreign/adjacent state. `make encryption-recovery-cleanup-test`; ADRs 0200–0206. Committed performance evidence remains |
...
Complete milestone 9.7 with reproducible native-versus-encrypted performance
evidence, including both families, latency distribution, resource cost, MTU,
convergence, scale, and rotation disruption.
