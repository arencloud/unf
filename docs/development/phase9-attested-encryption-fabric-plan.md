...
Complete milestone 9.5 by producing 9.5k facts from the agent's live Node-local
authority and implementing the local orchestrator that configures/read-backs
WireGuard and routes before issuing the 9.5h latch;
...
recovery, and authenticated complete-cut fact reconciliation contracts now pass
their focused gates, but no workload packet-path activation is claimed yet.
