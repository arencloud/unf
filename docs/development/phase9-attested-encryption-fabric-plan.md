...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0238 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration, recovery, non-perturbing evidence, causal-readiness, monotonic-platform-health, activation-supersession, immutable requalification, and bounded-witness boundaries. Live cl02 work exposed observer churn, a simultaneous five-Node expired Active-plus-Prepared key deadlock, a valid predecessor activation report fencing its successor, and unbounded streaming samples. Expired Authority Recovery renews unusable authority without plaintext fallback; Conflict-Acknowledged Activation Supersession retires only a controller-confirmed stale outbox; Node-local loopback observation cannot be obstructed by the qualified dataplane. Exact revised runtime `e1dbb94` passed fresh full Kind; the complete cl02 gate remains |
...
Node-local control witness. The OpenShift gate
