...
| 9.8 | Kube-proxy-free Kind qualification | **Verified** | Safety-fixed runtime and qualifier `1dbef4d` passed the fresh three-Node dual-stack Kubernetes v1.35.0 gate. Default-required and explicit-selective PodIP/Service traffic, ciphertext-only Required capture, fail-closed link loss, natural rotation, controlled agent and controller replacement, loss-free operations, performance, Phase 8 egress coexistence, exact cleanup, and no-CNI rollback passed. Evidence JSON SHA-256 is `d73bd08d…708`; packet-capture SHA-256 is `2f944751…ed7`. ADRs 0208–0212 and 0214 |
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0219 and `make encryption-phase9-openshift-gate-test` define a digest-pinned, explicitly acknowledged Native-to-Required migration, a Causal Pre-Attachment Fleet Barrier, a Monotonic Fleet Key Epoch Floor, reciprocal active-generation proof participation, forward cursor recovery, exact kernel self-healing, and demand-driven reconstruction of missing activation history for safe mixed-version, interrupted-preparation, and controller recovery. The five-Node dual-stack cl02 RHCOS/SELinux/CRI-O gate covers cross-worker PodIP/Service ciphertext, fail-closed selective traffic, rotation/recovery, loss-free operations, exact cleanup, convergence, kube-proxy absence, and ClusterOperator comparison. The corrected runtime must repeat Kind before live OpenShift execution |
