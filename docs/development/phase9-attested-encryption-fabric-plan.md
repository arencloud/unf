...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0225 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and recovery boundaries. Content-Verified Compact Frontier bounds and verifies the five-Node checkpoint; Complete Fleet Cursor Recovery emits one restart successor above the exact fleet maximum. The following Kind run reached controlled agent replacement after rotation and exposed a full `Draining` plus `Active` window against the next fleet floor. Retirement-Before-Catch-Up preserves that exact authority, retires only with positive zero-state proof, and then prepares the successor; bounded fenced retry absorbs temporary replacement-Pod watch latency without accepting denied authority. Focused tests pass. A fresh full Kind qualification, immutable publication, and the complete cl02 gate remain |
...
Run the complete fresh dual-stack Kind gate against ADRs 0223–0225, publish that exact
