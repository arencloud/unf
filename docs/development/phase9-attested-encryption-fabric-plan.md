...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0261 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and bounded recovery/evidence boundaries. Cut-Fenced Single-Flight Authority Admission applies readiness, fail-fast one-request concurrency, and a monotonic informer revision directly to internal port 9964. Exact `73057c1` passed fresh Kind and its public immutable images are pinned. Preserved-state deployment and the complete cl02 platform gate remain |
...
Deploy exact ADR 0261 successor `73057c1` to cl02 without deleting preserved
