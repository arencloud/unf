...
| 9.5 | Intent-Coalesced Cryptographic Fast Path | **In progress** | Phase 9.5a adds the canonical compiler and Causal Epoch Lease. Phase 9.5b adds the Causal Commit Vector and total recovery contract. Phase 9.5c adds the isolated four-map Aya ABI, Phase 9.5d adds its Proof-Carrying Aya Map Mirror, and Phase 9.5e adds the collision-free Cooperative Route-Mark Lease under focused gates and ADRs 0162–0166. Phase 9.5f adds Route-Before-Authority: canonical snapshot/rule witnesses, route-first masked IPv4/IPv6 rtnetlink activation, exact replay/cleanup/foreign preservation, and a generation-bound permit required by the Aya apply boundary under `make encryption-route-authority-test` plus its isolated live-kernel gate; ADR 0167. Controller distribution, TC consumption/verifier, and live encrypted traffic remain before Verified |
...
Complete milestone 9.5 by distributing authenticated compiled generations and
their 9.5f Route-Before-Authority evidence into the agent transaction boundary;
...
mirror plus cooperative mark and policy-route activation contracts now pass
their focused gates, but no workload packet-path activation is claimed yet. Live two-ended encrypted
