...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0269 and `make encryption-phase9-openshift-gate-test` define the qualification boundary. cl02 rejected `3026662` Required activation with per-plan full-contract duplication. Runtime `cfff9c3` completes compact authority and once-only integrity replay; immutable successor images are published with Kind pending. Exact cl02 deployment and complete qualification at the normal 2-GiB controller limit run first, then fresh Kind |
