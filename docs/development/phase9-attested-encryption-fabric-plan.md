...
| 9.9 | OpenShift qualification | **In progress** | ADRs 0213–0268 and `make encryption-phase9-openshift-gate-test` define the digest-pinned acknowledged migration and bounded recovery/evidence boundaries. Exact `3026662` passed fresh Kind and five-Node preserved-state deployment, but cl02 rejected its Required transition when the proof coordinator and endpoint duplicated a complete contract per selected plan. ADR 0268's schema-v1 contract table, lightweight indexed selections, once-only admission replay, logarithmic lookup, and 8-MiB legacy expansion fence are implemented. Fresh Kind, immutable successor publication, exact cl02 redeployment, and the complete platform gate remain |
