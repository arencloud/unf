...
| 9.7 | Operations, upgrade, recovery, and performance | **In progress** | Phases 9.7a–f provide causal evidence/export/persistence, acknowledged activation, minimum-causal-cut operations, and the Bidirectional Compatibility Sextant. Five additive all-or-zero encryption coordinates allow both adjacent rollout orders while partial/foreign state fails before BPF access and every endpoint remains exact-schema fenced. `make encryption-adjacent-compatibility-test`; ADRs 0200–0205. Outage/replacement/cleanup and committed performance evidence remain |
...
Continue milestone 9.7 with outage/replacement/exact-cleanup qualification,
then commit native-versus-encrypted performance evidence as an independent
verified slice.
