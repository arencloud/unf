...
| 9.9 | OpenShift qualification | **In progress** | ADR 0213 and `make encryption-phase9-openshift-gate-test` define a digest-pinned, explicitly acknowledged Native-to-Required migration plus five-Node dual-stack cl02 RHCOS/SELinux/CRI-O gate with cross-worker PodIP/Service ciphertext, fail-closed selective traffic, rotation/recovery, loss-free operations, exact cleanup, convergence, kube-proxy absence, and ClusterOperator comparison. Live deployment and execution remain |
