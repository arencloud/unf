...
| 9.8 | Kube-proxy-free Kind qualification | **In progress** | Phase 9.8a adds Content-Addressed Membership Resume across harmless topology/controller churn; 9.8b adds Secret-Aware Positive Rollback for exact checkpoint/key removal without pathname globbing. Node replacement and foreign state remain fenced; focused tests, strict Clippy, and a real interrupted three-Node cleanup pass; ADRs 0208–0209. One exact committed runtime must now pass the complete live gate |
