...
| 9.6 | Bidirectional live path proof | **In progress** | Phase 9.6a adds the Causal Duplex Path Quorum over exact contract, kernel, counter, transcript, identity, family, and expiry evidence. Phase 9.6b adds a Pull-Synchronized Duplex Proof Exchange derived atomically from the active fleet-plan generation: current Pod/Node-UID-authenticated endpoints see only their assignments, append-only proof ledgers cannot cross generation changes, expired rounds renew, and only involved endpoints receive current completed receipts. `make encryption-path-runtime-test`; ADRs 0194–0195. Agent-side live execution and consuming activation remain 9.6c |
