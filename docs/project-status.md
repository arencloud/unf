...
| Egress NetworkPolicy compatibility | **In progress** | Direction-aware IR/evaluation, `spec.egress` translation/defaulting, destination `ipBlock` semantics, source-side lowering, transactional map staging, TC lookup, controller distribution, focused dual-stack lifecycle/provenance, explanation/status, retained history, and NetworkPolicy what-if simulation are implemented. Controller-offline replacement with populated egress maps, stateful reply handling, and OpenShift qualification remain open |
