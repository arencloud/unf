...
| Egress NetworkPolicy compatibility | **In progress** | Direction-aware IR/evaluation, `spec.egress` translation/defaulting, destination `ipBlock` semantics, source-side lowering, transactional map staging, and TC lookup are implemented. Controller distribution, simulation/status integration, dual-stack kind lifecycle/provenance/recovery with populated egress maps, stateful reply handling, and OpenShift qualification remain open |
