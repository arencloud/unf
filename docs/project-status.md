...
| NetworkPolicy egress translation and defaulting | **Verified** | The multi-direction compiler emits independent ingress/egress IR, follows Kubernetes implicit and explicit `policyTypes`, maps selected source Pods to `to` destinations with shared peer/port parsing, and evaluates allow/default isolation. Egress `ipBlock` is explicitly rejected pending destination-address evaluation, while controller tests prove no egress IR can enter ingress-only snapshots or advance their revision; ADR 0032 records the contract |
| Egress NetworkPolicy compatibility | **In progress** | Direction-aware IR/evaluation and `spec.egress` translation/defaulting are verified behind the enforcement gate. Destination `ipBlock`, source-side lowering/maps and TC enforcement, controller distribution, simulation/status integration, dual-stack kind lifecycle/provenance/recovery, and OpenShift qualification remain open |
