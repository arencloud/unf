...
| NetworkPolicy lifecycle | Namespace relabel removed/restored the allow without identity churn; an unsupported named-port update removed stale compiled state and recovered; deletion allowed 9091 and recreation restored the drop |
...
| Deliverable | State | Exit evidence |
|---|---|---|
| NetworkPolicy ingress translator foundation | **Verified** | Unit tests cover allow/default isolation, pod/Namespace label expressions, wildcards, local/exact-namespace peers, and explicit unsupported-feature errors; `make kind-test` exercises expression-based target and peer selectors |
...
| Full ingress selector/port compatibility | **In progress** | Pod and Namespace label expressions are live verified; named/ranged ports, IP blocks, and broader conformance cases remain |
