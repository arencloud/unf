...
| Upstream-aligned ingress matrix | **Verified** | `hack/verify-networkpolicy-ingress.sh` maps supported upstream scenarios—including Pod/Namespace `matchExpressions`, multiple ingress rules, and label-driven recovery—to revision-converged cross-node traffic and explanation checks, then deletes its three exact test Namespaces and requires baseline policy counts; scope and exclusions are recorded in `docs/development/networkpolicy-conformance.md` |
