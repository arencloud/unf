...
| Egress NetworkPolicy compatibility | **In progress** | Direction-aware IR/evaluation, `spec.egress` translation/defaulting, and destination `ipBlock` semantics are verified behind the enforcement gate. Source-side lowering/maps and TC enforcement, controller distribution, simulation/status integration, dual-stack kind lifecycle/provenance/recovery, and OpenShift qualification remain open |
