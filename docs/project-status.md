...
| Physical map-pressure failure injection | **Planned** | Force a deterministic staging-map insertion failure at physical capacity, require rollback to preserve the selected active bank/revision and traffic, then prove a later valid update succeeds |
...
- Partial pin sets, malformed active policy config, and invalid inactive-stage
  values are live-verified as rejected using isolated bpffs fault sets. Capacity
  bounds are unit-covered, but physical map-exhaustion rollback has not yet been
  injected against a live staging map.
