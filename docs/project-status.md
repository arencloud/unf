...
| Agent state | Two ready agents with BPF loaded; both applied identity revision 63 and policy revision 139 in controller epoch 7677549857574155835, with 7 identity-map and 55 combined active policy-map entries; they exported 1,411 and 4,277 observations with zero queued/dropped at capture |
...
| Topology state | Schema v2 returned 2 ready Nodes, 17 placed workloads, 5 Services, and their EndpointSlice backends at topology revision 127/service revision 47; a selectorless `frontend/topology-probe` exposed a not-ready backend for `frontend/client`, changed it to ready, removed it on EndpointSlice deletion, then removed the Service, while policy revision remained unchanged |
| Flow history | Schema v1 retained 586 logical flows/5,719 observations at telemetry revision 793 in its 4,096-key bound, including 16 protocol-132 SCTP flows with enriched temporary client/server workload provenance; `frontend/client` → `backend/server` TCP/8080 remained enriched and attributed to `unf-dev-worker`, while both agents reported zero export drops |
...
| Policy simulation | Schema v2 evaluated 85 topology-derived flows at identity revision 63/policy revision 139/topology revision 127 and 554 of 586 retained historical flows/1,909 observations at history revision 793; it predicted one representative and 63 observed TCP/8080 denials, reported `backend/server` affected, explicitly skipped 32 stale-identity flows including cleaned-up conformance workloads, left live policy state unchanged, and TCP/8080 remained allowed |
...
| Full ingress peer/port compatibility | **In progress** | Pod/Namespace expressions, namespace-wide omitted targets, implicit ingress policy type for egress-omitted objects, default TCP protocol, non-selected Pod behavior, named and protocol-only TCP/UDP/SCTP ports, inclusive numeric ranges up to 1,024 ports, and IPv4 blocks up to 1,024 addresses with `except` are live verified; IPv6, unbounded blocks, and broader upstream conformance remain |
