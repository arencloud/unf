...
| Topology state | Schema v2 returned 2 ready Nodes, 17 placed workloads, 5 Services, and their EndpointSlice backends at topology revision 82/service revision 46; a selectorless `frontend/topology-probe` exposed a not-ready backend for `frontend/client`, changed it to ready, removed it on EndpointSlice deletion, then removed the Service, while policy revision remained unchanged |
| Flow history | Schema v1 retained 508 logical flows/3,355 observations at telemetry revision 379 in its 4,096-key bound; `frontend/client` → `backend/server` TCP/8080 was enriched and attributed to `unf-dev-worker`, while both agents reported zero export drops |
...
| NetworkPolicy lifecycle | Named port `allowed` resolved to TCP/8081; the inclusive 8082–8083 range enforced both boundaries and excluded 8084; a protocol-only TCP entry allowed arbitrary TCP/9091 without allowing UDP/9091 and removal restored isolation; an exact IPv4 block allowed the client, a nested exception denied it, and exception removal recovered; Namespace relabel removed/restored the allow without identity churn; oversized range/block updates removed stale state and recovered; deletion allowed 9091 and recreation restored the drop; omitted `podSelector`, `policyTypes`, and protocol produced namespace-wide/default-ingress/default-TCP behavior, while target narrowing restored non-isolated traffic |
| Policy explanation | Native 8080/9090, compatibility exact/range/protocol-only ports, bounded IPv4 block/exception transitions, and namespace-wide target/defaulting transitions reported the expected explicit/default/no-applicable-policy provenance with dataplane enforcement truthfully enabled |
| Policy simulation | Schema v2 evaluated 68 topology-derived flows at identity revision 30/policy revision 84/topology revision 82 and 500 of 508 retained historical flows/1,760 observations at history revision 380; it predicted one representative and 59 observed TCP/8080 denials, reported `backend/server` affected, explicitly skipped eight stale-identity flows, left live policy state unchanged, and TCP/8080 remained allowed |
...
| Deliverable | State | Exit evidence |
|---|---|---|
| NetworkPolicy ingress translator foundation | **Verified** | Unit tests cover allow/default isolation, omitted namespace-wide target/default ingress/default TCP, pod/Namespace expressions, named and protocol-only TCP/UDP ports, bounded numeric ranges, bounded IPv4 blocks/exceptions, wildcards, local/exact-namespace peers, and explicit unsupported-feature errors; `make kind-test` exercises each supported peer/port form |
...
| Full ingress peer/port compatibility | **In progress** | Pod/Namespace expressions, namespace-wide omitted targets, implicit ingress policy type for egress-omitted objects, default TCP protocol, non-selected Pod behavior, named and protocol-only TCP/UDP ports, inclusive numeric ranges up to 1,024 ports, and IPv4 blocks up to 1,024 addresses with `except` are live verified; IPv6, SCTP, unbounded blocks, and broader upstream conformance remain |
