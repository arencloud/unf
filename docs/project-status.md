...
| Egress NetworkPolicy compatibility | **In progress** | Direction-aware IR/evaluation, `spec.egress` translation/defaulting, destination `ipBlock` semantics, source-side lowering, transactional map staging, TC lookup, controller distribution, and focused dual-stack lifecycle/provenance are implemented. Direction-aware explanation, simulation/status/history integration, controller-offline replacement with populated egress maps, stateful reply handling, and OpenShift qualification remain open |
