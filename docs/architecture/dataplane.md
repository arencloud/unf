...
| `EGRESS_SOURCES` | source identity and bank | lease plus six revision domains, contract/intent commitments, shared intent index, candidate counts, admission and family/standby flags | egress compiler; agent transactional writer; source TC reader | inactive entries are replaced/read back before the shared egress pointer changes; persistent ABI v15 | 131,072 entries across two banks; an explicit source is only fenced or active |
| `EGRESS_DESTINATIONS_V4`, `EGRESS_DESTINATIONS_V6` | exact intent index and bank followed by destination prefix | contract revision, intent digest, monotonic new-flow and established-flow deadlines | egress compiler; agent transactional writer/reconstructor; source/gateway TC LPM readers | both families stage, read back, recover, roll back, and retire under persistent ABI v15; exact PLR addresses coexist with owned dual-stack deny catch-alls | 262,144 entries each; packet time enforces temporal authority without controller availability; static entries use an explicit infinite sentinel |
...
| `EGRESS_GATEWAY_NAT_SOURCES`, `EGRESS_GATEWAY_NAT_DESTINATIONS_V4/V6`, `EGRESS_GATEWAY_NAT_ADDRESSES`, `EGRESS_GATEWAY_NAT_GATEWAYS`, `EGRESS_GATEWAY_NAT_SELECTIONS` | source identity plus bank, followed by destination prefix, candidate, or bucket dimensions | exact heterogeneous contract/lease/intent/digest, local-primary gateway, address, and proof-witness projection | gateway compiler; agent transactional writer/reconstructor; gateway TC reader | seven dedicated maps stage/read back/recover/retire under one aggregate pointer in persistent ABI v15; identity namespacing prevents cross-contract revision/index aliasing | same bounded capacities as the source tables; malformed aggregate or owned state fails closed |
...
| `EGRESS_CONNECTIONS` | complete original or translated dual-stack L4 tuple, source identity, family, role | last-seen, contract/lease, original and translated tuples, chosen indexes, contract digest, proof witness, primary/standby gateway digests, established-flow deadline | gateway TC writer/reader; agent persistent owner | persistent ABI-v15 LRU; reverse key is reserved first and forward key second with `BPF_NOEXIST`; PLR pairs expire at the earlier protocol or monotonic authority bound | 262,144 entries; autonomous expiry removes authority during controller outage, while AFT schema v2 transports a conservatively re-anchored portable deadline |
| `EGRESS_FQDN_CONNECTIONS` | original source-side dual-stack L4 tuple and stable intent digest | last-seen plus monotonic established-flow deadline | source TC writer/reader | runtime-only LRU; created only for a temporal new TCP SYN or UDP tuple and reset on program replacement | 262,144 entries; only the remembered exact tuple drains after new-flow closure and expiry fails closed |
| `SERVICE_FRONTENDS_V4`, `SERVICE_FRONTENDS_V6` | exact address, network-order port, protocol, bank | ServiceId, revision-local frontend index, selected-tier slot count/code, ClientIP timeout/flag, actual StableHash/Maglev flag, schema, revision | verified-contract compiler; agent transactional writer; TC reader | both families and all origin-specific slot groups stage/read back together under ABI v15 | 262,144 physical entries each, capped at 131,072 per bank; an exact empty strict/fallback tier drops new flows while an unrelated tuple passes |
...
current ABI v15 link directory and atomically updates that link to the newly loaded
...
accepts only the known v1–v15 map names and numeric UNF TCX link-pin names,
refuses unknown directory content, and requires `--allow-current-abi` for v15. Legacy
...
v15 owns its source, destination, candidate and selection banks, source config,
dedicated aggregate gateway-NAT banks, and connection LRU; v14 remains an exact
historical 40-map boundary and v13 an exact historical 33-map boundary.
...
Phase 8.6 added live proof-gated HA without changing the then-current
persistent ABI-v14 map set. Phase 8.7c advances ownership to ABI v15 because
reserved bytes gain temporal meaning. Runtime-only
`SERVICE_DATAPLANE_TAIL_CALLS_V2` has eight slots, separating
