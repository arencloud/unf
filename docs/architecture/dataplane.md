...
closed; neither runtime map belongs to the persistent 25-map ABI-v11 set.
...
current ABI v11 link directory and atomically updates that link to the newly loaded
...
accepts only the known v1–v11 map names and numeric UNF TCX link-pin names,
refuses unknown directory content, and requires `--allow-current-abi` for v11. Legacy
