...
| `CONNECTIONS` | complete IPv4/IPv6 L4 tuple | last-seen time and admission revision | ingress and egress TC programs | runtime-only LRU; primary-CNI mode seeds pre/post-NAT tuples from both hooks; protocol-bounded and reset on program replacement | 65,536 entries; an entry requires active policy state but survives unrelated revision churn; misses fall through to current policy |
