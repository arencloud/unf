...
| `EGRESS_SOURCES` | source identity and bank | lease plus six revision domains, contract/intent commitments, shared intent index, candidate counts, admission and family/standby flags | egress compiler; agent transactional writer; source TC reader | inactive entries are replaced/read back before the shared egress pointer changes; persistent ABI v13 | 131,072 entries across two banks; an explicit source is only fenced or active |
| `EGRESS_DESTINATIONS_V4`, `EGRESS_DESTINATIONS_V6` | exact intent index and bank followed by destination prefix | contract revision, intent digest, schema | egress compiler; agent transactional writer/reconstructor; source TC LPM reader | both families stage, read back, recover, roll back, and retire with source/candidate state under one pointer in persistent ABI v13 | 262,144 entries each; prefix length is 64 plus the network prefix, so `Any` remains isolated to its intent and bank |
...
closed; neither runtime map belongs to the persistent 33-map ABI-v13 set.
...
current ABI v13 link directory and atomically updates that link to the newly loaded
...
accepts only the known v1–v13 map names and numeric UNF TCX link-pin names,
refuses unknown directory content, and requires `--allow-current-abi` for v13. Legacy
...
v13 owns its source, destination, candidate and selection banks, config pointer,
and future connection LRU; v12 remains an exact historical cleanup boundary.
