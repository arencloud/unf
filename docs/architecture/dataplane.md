...
| `EGRESS_SOURCES` | source identity and bank | lease plus six revision domains, contract/intent commitments, shared intent index, candidate counts, admission and family/standby flags | egress compiler; agent transactional writer; future TC reader | inactive entries are replaced/read back before the shared egress pointer changes; persistent ABI v12 | 131,072 entries across two banks; an explicit source is only fenced or active |
| `EGRESS_ADDRESSES`, `EGRESS_GATEWAYS`, `EGRESS_SELECTIONS` | shared intent index, candidate or bucket, family, bank | exact egress address witness; certified route/neighbor/tunnel path; or 251-bucket primary/pre-certified-standby choice | egress compiler; agent transactional writer; future TC reader | all three tables stage and recover with `EGRESS_SOURCES` under one bank | 131,072 addresses, 262,144 gateway paths, and 4,112,384 no-preallocate selections; any partial stage preserves the active pointer |
| `EGRESS_CONFIG` | constant `u32` slot 0 | controller/projection/contract/path revisions, four counts, ABI schema, active bank | agent writer; future TC reader | sole atomic pointer; restart validates exact counts and removes uncommitted inactive state | one entry; zero config removes orphan staging state |
| `EGRESS_CONNECTIONS` | complete original dual-stack L4 tuple, source identity, family, role | last-seen, contract/lease, original and translated tuples, chosen address/path indexes, primary/standby witnesses | future TC writer/reader; agent persistent owner | bounded persistent ABI-v12 LRU reserved for steering/NAT; currently empty and unconsumed | 262,144 entries; packet insertion and validation are a later Phase 8.5 gate |
| `SERVICE_FRONTENDS_V4`, `SERVICE_FRONTENDS_V6` | exact address, network-order port, protocol, bank | ServiceId, revision-local frontend index, selected-tier slot count/code, ClientIP timeout/flag, actual StableHash/Maglev flag, schema, revision | verified-contract compiler; agent transactional writer; TC reader | both families and all origin-specific slot groups stage/read back together under ABI v12 | 262,144 physical entries each, capped at 131,072 per bank; an exact empty strict/fallback tier drops new flows while an unrelated tuple passes |
...
| `LOAD_BALANCER_FRONTENDS_V4`, `LOAD_BALANCER_FRONTENDS_V6` | VIP address, network-order Service port, protocol, bank | ServiceId, dedicated per-Node frontend index, selected-tier slot count/code, ClientIP timeout/flag, actual StableHash/Maglev and explicit DSR flags, service/reachability/allocation revisions, referenced service bank, schema, policy/source-range flags | verified-contract LoadBalancer compiler; agent transactional writer; TC reader | both families stage and read back under an independent bank in persistent ABI v12 and reference disjoint coherent service slots | 262,144 physical entries each, capped at 131,072 per bank; collision or partial stage preserves the active bank |
| `LOAD_BALANCER_SOURCE_RANGES_V4`, `LOAD_BALANCER_SOURCE_RANGES_V6` | ServiceId, VIP bank, source IPv4/IPv6 prefix | exact service/reachability/allocation revisions and schema | LoadBalancer compiler; agent transactional writer/reconstructor; TC LPM reader | runtime-only tries stage/read back/rollback with VIP state and reconstruct from authenticated durable checkpoints before attachment; deliberately excluded from persistent ABI v12 ownership | 262,144 entries each; an enabled frontend with no coherent longest-prefix match drops with bounded provenance |
...
`SERVICE_DATAPLANE_TAIL_CALLS` is a runtime-only four-entry program array. The agent
loads IPv4/IPv6 policy stages and IPv4/IPv6 DSR FIB stages into it before either
main TC hook attaches. Per-CPU post-lookup scratch transfers only fixed-width
observation and mode flags between stages. Missing policy or DSR targets fail
closed; neither runtime map belongs to the persistent 31-map ABI-v12 set.
...
current ABI v12 link directory and atomically updates that link to the newly loaded
...
accepts only the known v1–v12 map names and numeric UNF TCX link-pin names,
refuses unknown directory content, and requires `--allow-current-abi` for v12. Legacy
...
Phase 8.5 now has a verified fixed-width egress ABI contract and persistent ABI
v12 owns its four banked tables, config pointer, and future connection LRU.
Explicit source entries are only fenced
