...
| `SERVICE_FRONTENDS_V4`, `SERVICE_FRONTENDS_V6` | exact address, network-order port, protocol, bank | ServiceId, revision-local frontend index, selected-tier backend count/code, ClientIP timeout/flag, schema, revision | verified-contract compiler; agent transactional writer; TC reader | both families and all origin-specific slot groups stage/read back together under ABI v9 | 262,144 physical entries each, capped at 131,072 per bank; an exact empty strict/fallback tier drops new flows while an unrelated tuple passes |
...
| `SERVICE_CONNECTIONS` | complete IPv4/IPv6 L4 tuple plus forward/reverse role | last-seen, service revision, client/frontend/backend tuples, Cluster translated-source tuple, selected tier and affinity outcome, ServiceId, BackendId, schema/flags | TC writer/reader | persistent ABI v9; reverse inserts first, forward second, partial failure removes reverse; lookups validate the complete peer tuple and refresh both entries before affinity is considered | bounded 262,144-entry LRU; valid TCP/UDP pairs survive desired-state churn and endpoint draining until protocol timeout; corrupt/expired pairs are removed |
| `SERVICE_AFFINITY` | original client/frontend address, frontend port, protocol, family, active service bank; source port zero; affinity role | last-seen, service revision, stable BackendId, selected slot/tier, schema | TC writer/reader | persistent ABI v9; bank+revision identifies one immutable eligible set, timeout refresh is exact, and stale/current-invalid records are removed before StableHash reselection | bounded 262,144-entry LRU; used only when the verified frontend requests ClientIP, while `None` creates no affinity state |
| `NODE_PORT_FRONTENDS_V4`, `NODE_PORT_FRONTENDS_V6` | local Node address, network-order NodePort, protocol, bank | ServiceId, dedicated frontend index, selected-tier backend count/code, ClientIP timeout/flag, service revision/bank, schema, external-policy flag | verified-contract compiler; agent transactional writer; ingress TC reader | both families stage/read back under a NodePort-specific bank and reference an origin-independent service slot namespace in ABI v9 | 262,144 physical entries each, capped at 131,072 per bank; external Local is resolved before soft topology and never aliases ClusterIP internal policy |
...
current ABI v9 link directory and atomically updates that link to the newly loaded
...
accepts only the known v1–v9 map names and numeric UNF TCX link-pin names,
refuses unknown directory content, and requires `--allow-current-abi` for v9. Legacy
