...
| `unf-state` | Revision snapshots, bounded flow-history contract, topology schema, and identity metadata | Transport or controller loops |
| `unf-controller` | Watches, desired-state/topology reconciliation, bounded telemetry retention, explanation, and read-only simulation orchestration | Packet parsing |
