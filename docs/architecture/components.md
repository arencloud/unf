...
| `unf-route` | Routing-provider abstraction, strict remote Node/block intent, native endpoint and cross-node route IR, typed kernel lifecycle/repair, scoped rollback, and provider-declared MTU derivation | IP allocation, link mutation, durable transactions, Kubernetes access, or policy enforcement |
...
| `unf-controller` | Watches, EndpointSlice-aware desired-state/topology reconciliation, explicit Node block assignment/distribution, bounded durable agent-report and flow-history checkpointing, non-blocking external HTTP flow handoff, time-window flow queries, explanation, and read-only simulation orchestration | Packet parsing |
| `unf-agent` | Capability detection, Aya lifecycle, events, non-blocking telemetry export, authenticated durable node-block adoption, and opt-in root-authenticated local CNI transaction service | Kubernetes policy semantics or CNI namespace mutation |
