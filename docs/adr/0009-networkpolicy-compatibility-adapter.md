...
multi-value Pod `In` with Namespace-name `NotIn`, homogeneous multiple-PodSelector
and heterogeneous peer OR, explicit empty-list wildcards, multi-port OR, all
selector operators, Pod/Namespace label recovery, stacked additive allows, and
allow-all deletion recovery. Direct IPv4 and IPv6 Pod traffic now exercises every transition,
