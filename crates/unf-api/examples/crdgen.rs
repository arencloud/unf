...
use unf_api::{
    ...
    EgressReachabilityPlan, EncryptionPolicy, SecurityPolicy,
};

fn main() {
    let kind = std::env::args().nth(1).unwrap_or_else(|| {
        eprintln!(
            "usage: crdgen <security-policy|encryption-policy|egress-pool|egress-policy|egress-internet-classification|egress-reachability-plan|egress-reachability-observation>"
        );
        std::process::exit(2);
    });
    let crd = match kind.as_str() {
        "security-policy" => SecurityPolicy::crd(),
        "encryption-policy" => EncryptionPolicy::crd(),
