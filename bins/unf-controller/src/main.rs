...
#[derive(Debug, Clone, PartialEq, Eq)]
struct PendingEgressSourceApplication {
...
fn reconcile_egress_control_plane_locked(state: &ControllerState) -> Result<()> {
    ...
    let (candidates, expanded_candidates) =
        egress_ha_candidate_sets(state, &current_control_checkpoint, revision, &model)?;
    ...
    let mut next = reconcile_egress_ha_candidates(
        &control_plane,
        revision,
        &model,
        &explicit_providers,
    ...
    let final_checkpoint = next.checkpoint();
    let changed = final_checkpoint != before;
    *control_plane = next;
    drop(control_plane);
    if changed {
        state
        ...
        info!(
            ...
            allocated = final_checkpoint.allocation.leases.len(),
            ensuring = final_checkpoint
                .gateways
                .records
                .iter()
                .filter(|record| record.desired.action == unf_egress::EgressGatewayAction::Ensure)
                .count(),
            withdrawing = final_checkpoint
                .gateways
                .records
                .iter()
                .filter(|record| record.desired.action == unf_egress::EgressGatewayAction::Withdraw)
...
fn reconcile_egress_ha_candidates(
    current: &EgressControlPlane,
    revision: Revision,
    model: &EgressModel,
    explicit_providers: &BTreeMap<EgressIntentOwner, EgressProviderRef>,
    frozen: Vec<EgressHaCandidate>,
    expansion: Option<Vec<EgressHaCandidate>>,
) -> Result<EgressControlPlane> {
    let before = current.checkpoint();
    let attempted_expansion = expansion.is_some();
    let mut next = current.clone();
    next.reconcile_with_ha_candidates(
        revision,
        model.clone(),
        explicit_providers,
        expansion.unwrap_or_else(|| frozen.clone()),
    )
    .context("derive allocation and gateway desired state")?;
    if attempted_expansion && !egress_ha_assignments_unchanged(&before, &next.checkpoint()) {
        let mut retained = current.clone();
        retained
            .reconcile_with_ha_candidates(revision, model.clone(), explicit_providers, frozen)
            .context("retain frozen egress HA ownership after disruptive candidate expansion")?;
        debug!("deferred egress HA candidate expansion because it would move active shards");
        Ok(retained)
    } else {
        if attempted_expansion {
            info!("admitted zero-disruption egress HA candidate expansion");
        }
        Ok(next)
    }
}
...
fn retain_node_egress_distributions(state: &ControllerState, live_nodes: &BTreeSet<String>) {
    ...
    write_lock(&state.egress_pending_source_applications)
...
fn egress_application_counts(state: &ControllerState) -> (usize, usize, usize) {
    let source_nodes = mutex_lock(&state.egress_gateway_distributions)
        .admitted_sources
        .keys()
        .cloned()
        .collect::<Vec<_>>();
    let ready = source_nodes
        .iter()
        .filter(|node| egress_source_activation_ready(state, node))
        .count();
    (
        source_nodes.len(),
        read_lock(&state.egress_gateway_applications).len(),
        ready,
    )
}
...
fn egress_source_projection_for(
    ...
    let admitted = envelope
        ...
        .map_err(|error| egress_distribution_api_error(&error))?;
    write_lock(&state.egress_pending_source_applications).insert(
        agent.node_name.clone(),
        PendingEgressSourceApplication {
            agent: agent.clone(),
            projection: admitted,
        },
    );
...
async fn acknowledge_egress_source_application(
    State(state): State<Arc<ControllerState>>,
    headers: HeaderMap,
    Json(acknowledgement): Json<EgressSourceApplicationAcknowledgement>,
) -> Result<StatusCode, ApiError> {
    let agent = authenticate_internal_agent(&state, &headers).await?;
    acknowledge_egress_source_application_for(&state, &agent, &acknowledgement)?;
    Ok(StatusCode::NO_CONTENT)
}

fn acknowledge_egress_source_application_for(
    state: &ControllerState,
    agent: &AuthenticatedAgent,
    acknowledgement: &EgressSourceApplicationAcknowledgement,
) -> Result<(), ApiError> {
    let _guard = mutex_lock(&state.egress_distribution_guard);
    let pending = read_lock(&state.egress_pending_source_applications)
        .get(&agent.node_name)
        .cloned()
        .ok_or_else(|| {
            ApiError::service_unavailable("no issued source projection awaits acknowledgement")
        })?;
...
async fn egress_source_activation(
    State(state): State<Arc<ControllerState>>,
    headers: HeaderMap,
) -> Result<Response, ApiError> {
    let agent = authenticate_internal_agent(&state, &headers).await?;
    match egress_source_activation_grant_for(&state, &agent)? {
        Some(grant) => Ok(Json(grant).into_response()),
        None => Ok(StatusCode::NO_CONTENT.into_response()),
    }
}
...
fn egress_ha_candidate_sets(
    state: &ControllerState,
    checkpoint: &EgressControlPlaneCheckpoint,
    desired_revision: Revision,
    desired_model: &EgressModel,
) -> Result<(Vec<EgressHaCandidate>, Option<Vec<EgressHaCandidate>>)> {
    let frozen = live_egress_ha_candidates(state, checkpoint)?;
    let expansion = if desired_revision == checkpoint.desired_revision
        && desired_model == &checkpoint.desired_model
        && checkpoint.ha_promotions.is_empty()
        && !checkpoint.ha_plans.is_empty()
    {
        zero_disruption_egress_ha_expansion_candidates(state, checkpoint, &frozen)?
    } else {
        None
    };
    Ok((frozen, expansion))
...
fn live_egress_ha_candidates(
    ...
    eligible_egress_ha_candidates(&nodes, &node_uids, &capabilities)
}

fn eligible_egress_ha_candidates(
    nodes: &BTreeMap<String, TopologyNode>,
    node_uids: &BTreeMap<String, NodePortNodeRecord>,
    capabilities: &BTreeSet<EgressCapability>,
) -> Result<Vec<EgressHaCandidate>> {
    nodes
        .values()
        .filter(|node| {
...
fn zero_disruption_egress_ha_expansion_candidates(
    state: &ControllerState,
    checkpoint: &EgressControlPlaneCheckpoint,
    frozen: &[EgressHaCandidate],
) -> Result<Option<Vec<EgressHaCandidate>>> {
    if frozen.len() < 2
        || checkpoint
            .ha_plans
            .iter()
            .any(|plan| plan.candidates != frozen)
    {
        return Ok(None);
    }
    let nodes = read_lock(&state.nodes);
    let node_uids = read_lock(&state.node_port_nodes);
    let capabilities = frozen[0].node.capabilities.clone();
    let mut eligible = eligible_egress_ha_candidates(&nodes, &node_uids, &capabilities)?;
    if eligible.len() <= frozen.len()
        || !frozen.iter().all(|current| {
            eligible.iter().any(|candidate| {
                candidate.node == current.node
                    && candidate.capacity_units == current.capacity_units
                    && candidate.failure_domains == current.failure_domains
            })
        })
    {
        return Ok(None);
    ...
    for candidate in &mut eligible {
        candidate.standby = frozen
            .iter()
            .find(|current| current.node == candidate.node)
            .is_none_or(|current| current.standby);
    }
    Ok(Some(eligible))
}

fn egress_ha_assignments_unchanged(
    before: &EgressControlPlaneCheckpoint,
    after: &EgressControlPlaneCheckpoint,
) -> bool {
    before.ha_plans.iter().all(|previous| {
        after
            .ha_plans
            .iter()
            .find(|candidate| candidate.owner == previous.owner)
            .is_some_and(|candidate| candidate.assignments == previous.assignments)
    })
}
...
fn egress_gateway_projection_for(
    ...
    let admitted = projection
        ...
        .admit(&principal, advertisement)
        .map_err(|error| egress_distribution_api_error(&error))?;
    write_lock(&state.egress_pending_gateway_applications).insert(
        agent.node_name.clone(),
        PendingEgressGatewayApplication {
            agent: agent.clone(),
            projection: admitted,
        },
    );
...
async fn acknowledge_egress_gateway_application(
    State(state): State<Arc<ControllerState>>,
    headers: HeaderMap,
    Json(acknowledgement): Json<EgressGatewayApplicationAcknowledgement>,
) -> Result<StatusCode, ApiError> {
    let agent = authenticate_internal_agent(&state, &headers).await?;
    acknowledge_egress_gateway_application_for(&state, &agent, &acknowledgement)?;
    Ok(StatusCode::NO_CONTENT)
}

fn acknowledge_egress_gateway_application_for(
    state: &ControllerState,
    agent: &AuthenticatedAgent,
    acknowledgement: &EgressGatewayApplicationAcknowledgement,
) -> Result<(), ApiError> {
    ...
    let pending = read_lock(&state.egress_pending_gateway_applications)
        ...
        .cloned()
        .ok_or_else(|| {
            ApiError::service_unavailable("no issued gateway projection awaits acknowledgement")
        })?;
    if pending.agent != *agent {
        return Err(ApiError::forbidden(
    ...
    acknowledgement
        .verify(&pending.projection)
        .map_err(|error| ApiError::bad_request(error.to_string()))?;
    write_lock(&state.egress_gateway_applications).insert(
        agent.node_name.clone(),
        AppliedEgressGatewayApplication {
            agent: agent.clone(),
            acknowledgement: acknowledgement.clone(),
        },
    );
...
fn egress_source_activation_grant_for(
    state: &ControllerState,
    agent: &AuthenticatedAgent,
) -> Result<Option<EgressSourceActivationGrant>, ApiError> {
    let grant = egress_source_activation_grant(state, &agent.node_name, Some(agent))?;
    Ok(grant)
}

fn egress_source_activation_grant(
    state: &ControllerState,
    source_node: &str,
    requesting_agent: Option<&AuthenticatedAgent>,
) -> Result<Option<EgressSourceActivationGrant>, ApiError> {
    let _guard = mutex_lock(&state.egress_distribution_guard);
    let sources = mutex_lock(&state.egress_gateway_distributions);
    let Some(source) = sources.admitted_sources.get(source_node) else {
        return Ok(None);
    };
    let pending_sources = read_lock(&state.egress_pending_source_applications);
    let Some(pending_source) = pending_sources.get(source_node) else {
        return Ok(None);
    };
    if pending_source.projection != *source
    ...
    let applied_gateways = read_lock(&state.egress_gateway_applications);
    let mut applications = BTreeMap::new();
    for plan in &source.projection().contract.plans {
        for gateway in &plan.gateways {
            let Some(pending) = pending_gateways.get(&gateway.node.name) else {
                return Ok(None);
            };
            let Some(applied) = applied_gateways.get(&gateway.node.name) else {
                return Ok(None);
            };
            if pending.agent != applied.agent
                || !agent_application_is_current(state, &applied.agent)
                || applied.acknowledgement.verify(&pending.projection).is_err()
                || pending.projection.projection().is_withdrawal()
                || !pending
                    .projection
                    .projection()
                    .source_contracts
                    .contains(&source.projection().contract)
            {
                return Ok(None);
            }
            applications.insert(
                (gateway.node.name.clone(), gateway.node.uid.clone()),
                applied.acknowledgement.clone(),
            );
        }
    }
    ...
    EgressSourceActivationGrant::issue(source, applications.into_values().collect())
        .map(Some)
        .map_err(|error| egress_distribution_api_error(&error))
}

fn egress_source_activation_ready(state: &ControllerState, source_node: &str) -> bool {
    egress_source_activation_grant(state, source_node, None)
        .ok()
        .flatten()
        .is_some()
}

fn agent_application_is_current(state: &ControllerState, agent: &AuthenticatedAgent) -> bool {
    read_lock(&state.pods)
        .get(&format!("{}/{}", agent.namespace, agent.pod_name))
        .is_some_and(|pod| {
            pod.uid == agent.pod_uid
                && pod.node_name.as_deref() == Some(agent.node_name.as_str())
                && pod.endpoint.service_account == agent.service_account
        })
}
...
mod tests {
    ...
    fn encryption_plan_poll_projects_one_causal_catalog_and_coalesces_retries() {
        ...
        *mutex_lock(&state.revisions) = RevisionSet {
        ...
        let held_request =
            NodeLocalPlanRequest::issue("worker-a".to_owned(), None, [25; 32]).unwrap();
        let held = encryption_plan_for(&state, &agent, &held_request)
            .unwrap()
            .unwrap();
        assert_eq!(held.snapshot.generation, first.snapshot.generation);
        assert_eq!(held.snapshot.service_revision, Revision::new(5));
    ...
    fn egress_gateway_distribution_requires_authenticated_source_admission_and_withdraws() {
        ...
        assert!(
            egress_gateway_projection_for(&state, &gateway, &advertisement)
                .expect("source fetch is not application")
                .is_none()
        );
        let pending_source = read_lock(&state.egress_pending_source_applications)
            .get("worker-a")
            .expect("source application is pending")
            .projection
            .clone();
        let source_ack = EgressSourceApplicationAcknowledgement::issue(
            &pending_source,
            1,
            pending_source.projection().contract.plans.len(),
        )
        .expect("exact source application acknowledgement");
        assert_source_ack_rejects_replacement_pod(&state, &source, &source_ack);
        acknowledge_egress_source_application_for(&state, &source, &source_ack)
            .expect("source application admitted");
        acknowledge_egress_source_application_for(&state, &source, &source_ack)
            .expect("exact source acknowledgement replay is idempotent");
        let (active, gateway_ack) =
            acknowledge_gateway_projection(&state, &gateway, &advertisement);
        record_current_egress_agents(&state, &[&source, &gateway]);
        assert!(!egress_source_activation_ready(&state, "worker-a"));

        let gateway_b = authenticated_egress_agent("gateway-b");
        acknowledge_gateway_projection(&state, &gateway_b, &advertisement);
        record_current_egress_agents(&state, &[&gateway_b]);
        assert!(egress_source_activation_ready(&state, "worker-a"));
        let grant = egress_source_activation_grant_for(&state, &source)
            .expect("activation grant evaluation")
            .expect("all selected gateways applied the source contract");
        grant
            .verify(&pending_source)
            .expect("grant binds the exact pending source projection");
        assert_eq!(grant.gateway_applications.len(), 2);
        let mut replacement_source = source.clone();
        replacement_source.pod_uid = "replacement-source-uid".to_owned();
        assert!(
            egress_source_activation_grant_for(&state, &replacement_source)
                .expect("replacement activation evaluation")
                .is_none(),
            "a replacement Pod cannot inherit an activation grant"
        );
        assert_eq!(egress_application_counts(&state), (1, 2, 1));

        let mut stale_gateway_ack = gateway_ack.clone();
        stale_gateway_ack.projection_revision = Revision::new(99);
        assert_eq!(
            acknowledge_egress_gateway_application_for(&state, &gateway, &stale_gateway_ack)
                .expect_err("stale gateway application fails closed")
                .status,
            StatusCode::BAD_REQUEST
        );

        assert_application_invalidation_and_withdrawal(
            &state,
            &source,
            &gateway,
            &advertisement,
    ...
    fn acknowledge_gateway_projection(
        state: &ControllerState,
        gateway: &AuthenticatedAgent,
        advertisement: &EgressAgentAdvertisement,
    ) -> (
        EgressGatewayProjection,
        EgressGatewayApplicationAcknowledgement,
    ) {
        let projection = egress_gateway_projection_for(state, gateway, advertisement)
        ...
        let pending = read_lock(&state.egress_pending_gateway_applications)
            .get(&gateway.node_name)
            .expect("gateway application is pending")
            .projection
            .clone();
        let acknowledgement = EgressGatewayApplicationAcknowledgement::issue(&pending)
            .expect("exact gateway application acknowledgement");
        assert_gateway_ack_rejects_replacement_pod(state, gateway, &acknowledgement);
        acknowledge_egress_gateway_application_for(state, gateway, &acknowledgement)
            .expect("gateway application admitted");
        acknowledge_egress_gateway_application_for(state, gateway, &acknowledgement)
            .expect("exact gateway acknowledgement replay is idempotent");
        (projection, acknowledgement)
    }

    fn record_current_egress_agents(state: &ControllerState, agents: &[&AuthenticatedAgent]) {
        for agent in agents {
            let mut pod = pod_record(1, "unf-system", &agent.pod_name, "unf-agent");
            pod.uid = agent.pod_uid.clone();
            pod.node_name = Some(agent.node_name.clone());
            pod.endpoint.service_account = EGRESS_AGENT_SERVICE_ACCOUNT.to_owned();
            write_lock(&state.pods).insert(format!("{}/{}", agent.namespace, agent.pod_name), pod);
        }
    }

    fn assert_source_ack_rejects_replacement_pod(
        state: &ControllerState,
        source: &AuthenticatedAgent,
        acknowledgement: &EgressSourceApplicationAcknowledgement,
    ) {
        let mut replacement = source.clone();
        replacement.pod_uid = "replacement-source-uid".to_owned();
        assert_eq!(
            acknowledge_egress_source_application_for(state, &replacement, acknowledgement)
                .expect_err("a replacement Pod cannot inherit source application")
                .status,
            StatusCode::FORBIDDEN
        );
    }

    fn assert_gateway_ack_rejects_replacement_pod(
        state: &ControllerState,
        gateway: &AuthenticatedAgent,
        acknowledgement: &EgressGatewayApplicationAcknowledgement,
    ) {
        let mut replacement = gateway.clone();
        replacement.pod_uid = "replacement-gateway-uid".to_owned();
        assert_eq!(
            acknowledge_egress_gateway_application_for(state, &replacement, acknowledgement)
                .expect_err("a replacement Pod cannot inherit gateway application")
                .status,
            StatusCode::FORBIDDEN
        );
    }

    fn assert_application_invalidation_and_withdrawal(
        state: &ControllerState,
        source: &AuthenticatedAgent,
        gateway: &AuthenticatedAgent,
        advertisement: &EgressAgentAdvertisement,
        source_ack: &EgressSourceApplicationAcknowledgement,
        gateway_ack: &EgressGatewayApplicationAcknowledgement,
        active_revision: Revision,
    ) {
        invalidate_egress_distributions(state);
        assert!(!egress_source_activation_ready(state, "worker-a"));
        assert!(read_lock(&state.egress_gateway_applications).is_empty());
        assert_eq!(
            acknowledge_egress_source_application_for(state, source, source_ack)
                .expect_err("invalidated source evidence cannot replay")
                .status,
            StatusCode::SERVICE_UNAVAILABLE
        );
        ...
        assert!(withdrawn.revision > active_revision);
        assert_eq!(
            acknowledge_egress_gateway_application_for(state, gateway, gateway_ack)
                .expect_err("active evidence cannot acknowledge a withdrawal")
                .status,
            StatusCode::BAD_REQUEST
        );
        let pending = read_lock(&state.egress_pending_gateway_applications)
            .get("gateway-a")
            .expect("withdrawal application is pending")
            .projection
            .clone();
        let acknowledgement = EgressGatewayApplicationAcknowledgement::issue(&pending)
            .expect("exact withdrawal acknowledgement");
        acknowledge_egress_gateway_application_for(state, gateway, &acknowledgement)
            .expect("withdrawal application admitted");
        assert!(
            read_lock(&state.egress_gateway_applications)["gateway-a"]
                .acknowledgement
                .withdrawn
        );
        assert_eq!(egress_application_counts(state), (0, 1, 0));
    }
    ...
    fn native_egress_policy() -> EgressPolicy {
        native_egress_policy_with_addresses(2)
    }

    fn native_egress_policy_with_addresses(addresses_per_family: u16) -> EgressPolicy {
        serde_json::from_value(serde_json::json!({
            "apiVersion": "network.unf.io/v1alpha1",
            "kind": "EgressPolicy",
            ...
            "spec": {
                ...
                "egress": {
                    "pool": "finance",
                    "families": ["IPv4"],
                    "addressesPerFamily": addresses_per_family
                }
            }
        }))
    ...
    #[test]
    fn recovered_egress_gateway_rejoins_only_without_active_shard_movement() {
        let build_state = |addresses_per_family: u16| {
            let state = new_state(true);
            for (name, suffix) in [("gateway-a", 10), ("gateway-b", 11)] {
                apply_node_event(&state, Event::Apply(egress_gateway_node(name, suffix)));
            }
            apply_egress_pool_event(&state, Event::Apply(native_egress_pool("192.0.2.0/24")));
            apply_egress_policy_event(
                &state,
                Event::Apply(native_egress_policy_with_addresses(addresses_per_family)),
            );
            state
        };
        ...
        let before = mutex_lock(&state.egress_control_plane).checkpoint();
        assert_eq!(before.ha_plans[0].candidates.len(), 2);
        let assignments = before.ha_plans[0].assignments.clone();
        apply_node_event(&state, Event::Apply(egress_gateway_node("gateway-c", 12)));
        let expanded = mutex_lock(&state.egress_control_plane).checkpoint();
        assert_eq!(expanded.ha_plans[0].candidates.len(), 3);
        assert_eq!(
            expanded.ha_plans[0]
                .candidates
                .iter()
                .filter(|candidate| candidate.standby)
                .count(),
            1
        );
        assert_eq!(expanded.ha_plans[0].assignments, assignments);
        assert_eq!(expanded.gateways.records[0].desired.nodes.len(), 3);
    ...
    fn egress_gateway_node(name: &str, suffix: u8) -> Node {
        let mut node = node(true);
        node.metadata.name = Some(name.to_owned());
        node.metadata.uid = Some(format!("{name}-uid"));
        let labels = node.metadata.labels.get_or_insert_default();
        labels.insert("kubernetes.io/hostname".to_owned(), name.to_owned());
        labels.insert(
            PRIMARY_CNI_NODE_LABEL.to_owned(),
            PRIMARY_CNI_NODE_LABEL_VALUE.to_owned(),
        );
        ...
        if let Some(status) = node.status.as_mut() {
            status.addresses = Some(vec![
                k8s_openapi::api::core::v1::NodeAddress {
                    address: format!("192.0.2.{suffix}"),
                    type_: "InternalIP".to_owned(),
                },
                k8s_openapi::api::core::v1::NodeAddress {
                    address: format!("fdff::{suffix:x}"),
                    type_: "InternalIP".to_owned(),
                },
            ]);
        }
