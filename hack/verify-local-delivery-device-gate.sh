...
jq -en -L "$root/hack" '
  ...
  def snapshot($index; $device; $packets; $overlimits; $bind):
    [{"total acts":0},{actions:[{kind:"mirred",mirred_action:"redirect",direction:"egress",
      ...
      bind:$bind,ref:($bind+1),skip_hw:true}]}];
  ...
  ([range(0;8) as $stage | range(0;14) as $mutation |
    ($good |
      ...
      elif $mutation == 7 then .[$stage][1].actions[0].skip_hw=false
      ...
      elif $mutation == 11 then .[$stage][1].actions[0].control_action.type="pipe"
      elif $mutation == 12 then .[$stage][1].actions[0].skip_sw=true
      else .[$stage][1].actions[0].in_hw=true end
      | device_lifetime_verified)] | all(.==false)) and
  ([null,{},[],[$good[0]],($good + [$good[0]]),($good|reverse)] |
     all(.[]; device_lifetime_verified == false))' >/dev/null
echo 'Device lifetime evidence gate: positive, 112 field mutations and six malformed/reordered cuts passed'
