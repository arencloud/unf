...
for replacement_node in "${client_node}" "${server_node}"; do
    ...
    sudo "${container_runtime}" exec "${replacement_node}" sh -ec '
        ...
        test -e /sys/fs/bpf/unf/v10/SERVICE_CONFIG
        test -e /sys/fs/bpf/unf/v10/SERVICE_FRONTENDS_V4
        test -e /sys/fs/bpf/unf/v10/SERVICE_FRONTENDS_V6
        test -e /sys/fs/bpf/unf/v10/SERVICE_BACKENDS_V4
        test -e /sys/fs/bpf/unf/v10/SERVICE_BACKENDS_V6
        test -e /sys/fs/bpf/unf/v10/NODE_PORT_CONFIG
        test -e /sys/fs/bpf/unf/v10/NODE_PORT_FRONTENDS_V4
        test -e /sys/fs/bpf/unf/v10/NODE_PORT_FRONTENDS_V6
    '
done
...
spec:
  ...
  containers:
    - name: audit
      ...
      args:
        - |
          test "\$(bpftool -j map dump pinned /sys/fs/bpf/unf/v10/NODE_PORT_FRONTENDS_V4 | jq length)" -eq 0
          test "\$(bpftool -j map dump pinned /sys/fs/bpf/unf/v10/NODE_PORT_FRONTENDS_V6 | jq length)" -eq 0
          snapshot=/var/lib/unf/cni/v1/service-snapshot.json
