...
required_reply_locality_wait() {
    ...
    while (( SECONDS < deadline )); do
        ...
        for node in "$source_node" "$destination_node"; do
            ...
            if [[ $mode == required || $locality_acquisition == all-plans ]]; then
                ...
                [[ $mode == required ]] || minimum=0
                if ! jq -L "$project_root/hack" -e --arg node "$node" --arg uid "$uid" --arg cluster "$locality_cluster_uid" \
                    --arg acquisition "$locality_acquisition" --argjson minimum "$minimum" --slurpfile plan "$attempt_dir/$node-plan.json" --slurpfile agents "$attempt_dir/agents.json" '
                    ...
                    and (if $acquisition=="all-plans" then .acquisition=="allAdmittedPlans" else true end)
            ...
            if [[ ${require_locality_inventory:-false} == true ]]; then
                ...
                if ! jq -L "$project_root/hack" -e --arg mode "$mode" \
                    ...
                    and (if $acquisition == "all-plans" then locality_all_plan_inventory_valid($minimum)
                         elif $mode == "required" then locality_inventory_valid($minimum)
