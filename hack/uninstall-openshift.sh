...
for index in "${!nodes[@]}"; do
    ...
    cleanup_plan=$("${kc[@]}" -n unf-system exec "${pod}" -- \
        ...
        --abi-version 10 --allow-current-abi \
    ...
    grep -q 'ABI directory: /sys/fs/bpf/unf/v10' <<<"${cleanup_plan}"
    [[ $(grep -c 'remove map pin: /sys/fs/bpf/unf/v10/' <<<"${cleanup_plan}") -eq 25 ]]
...
echo "  verify /sys/fs/bpf/unf/v4 through /sys/fs/bpf/unf/v10 and UNF legacy filters are absent"
