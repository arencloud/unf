...
for replacement_node in "${client_node}" "${server_node}"; do
    ...
    sudo "${container_runtime}" exec "${replacement_node}" sh -ec '
        ...
        test -e /sys/fs/bpf/unf/v12/LOAD_BALANCER_CONFIG
        test -e /sys/fs/bpf/unf/v12/LOAD_BALANCER_FRONTENDS_V4
        test -e /sys/fs/bpf/unf/v12/LOAD_BALANCER_FRONTENDS_V6
        test ! -e /sys/fs/bpf/unf/v12/LOAD_BALANCER_SOURCE_RANGES_V4
        test ! -e /sys/fs/bpf/unf/v12/LOAD_BALANCER_SOURCE_RANGES_V6
    '
done
      ...
      args:
        - |
          for map in LOAD_BALANCER_FRONTENDS_V4 LOAD_BALANCER_FRONTENDS_V6; do
            test "\$(bpftool -j map dump pinned /sys/fs/bpf/unf/v12/\$map | jq length)" -eq 0
          done
