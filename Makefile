...
encryption-contract-test: encryption-fabric-boundary-test
	hack/verify-encryption-contract.sh
	cargo test -p unf-encryption
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-key-authority-test: encryption-contract-test
	hack/verify-encryption-key-authority.sh
	cargo test -p unf-encryption key_authority
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-kernel-provider-test: encryption-key-authority-test
	hack/verify-encryption-kernel-provider.sh
	cargo test -p unf-encryption kernel_provider
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-kernel-provider-live-test: encryption-kernel-provider-test
	hack/verify-encryption-kernel-live.sh

encryption-fast-path-contract-test: encryption-kernel-provider-test
	hack/verify-encryption-fast-path-contract.sh
	cargo test -p unf-ebpf-common
	cargo test -p unf-encryption fast_path
	cargo clippy -p unf-ebpf-common -p unf-encryption --all-targets --all-features -- -D warnings

encryption-fast-path-transaction-test: encryption-fast-path-contract-test
...
encryption-map-transaction-test: encryption-map-persistence-test
	hack/verify-encryption-map-transaction.sh
	cargo test -p unf-encryption proof_carrying_map_mirror
	cargo test -p unf-agent encryption_
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-map-backend-live-test: encryption-map-transaction-test
	hack/verify-encryption-map-backend-live.sh

encryption-route-mark-contract-test: encryption-map-transaction-test
	hack/verify-encryption-route-mark-contract.sh
	cargo test -p unf-ebpf-common encryption_route_mark
	cargo test -p unf-encryption cooperative_mark_lease
	cargo test -p unf-agent fixed_width_encoding_matches_the_shared_encryption_abi
	cargo clippy -p unf-ebpf-common -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-route-authority-test: encryption-route-mark-contract-test
	hack/verify-encryption-route-authority.sh
	cargo test -p unf-encryption route_before_authority
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-route-authority-live-test: encryption-route-authority-test
	unshare -Urn cargo test -p unf-encryption route_authority::linux::tests::privileged_route_before_authority_is_exact_replayable_and_foreign_safe -- --ignored --exact --nocapture

encryption-generation-distribution-test: encryption-route-authority-test
	hack/verify-encryption-generation-distribution.sh
	cargo test -p unf-encryption node_sealed_generation_capsule
...
encryption-generation-frontier-test: encryption-activation-latch-test
	hack/verify-encryption-generation-frontier.sh
	cargo test -p unf-encryption causal_generation_frontier
	cargo test -p unf-controller encryption_generation_delivery
	cargo clippy -p unf-encryption -p unf-controller --all-targets --all-features -- -D warnings

encryption-generation-recovery-test: encryption-generation-frontier-test
	hack/verify-encryption-generation-recovery.sh
	cargo test -p unf-encryption causal_generation_frontier
	cargo test -p unf-controller encryption_generation_
	cargo clippy -p unf-encryption -p unf-controller --all-targets --all-features -- -D warnings

encryption-generation-reconciler-test: encryption-generation-recovery-test
	hack/verify-encryption-generation-reconciler.sh
	cargo test -p unf-encryption complete_cut_fact_reconciler
	cargo test -p unf-controller encryption_generation_
	cargo clippy -p unf-encryption -p unf-controller --all-targets --all-features -- -D warnings

encryption-local-proof-ladder-test: encryption-generation-reconciler-test
	hack/verify-encryption-local-proof-ladder.sh
	cargo test -p unf-encryption causal_proof_ladder
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-linux-convergence-test: encryption-local-proof-ladder-test
	hack/verify-encryption-linux-convergence.sh
	cargo test -p unf-encryption linux_convergence_capsule
	cargo test -p unf-encryption kernel_plan_receives_only_the_exact_ready_local_epoch
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-agent-anti-entropy-test: encryption-linux-convergence-test
	hack/verify-encryption-agent-anti-entropy.sh
	cargo test -p unf-encryption linux_convergence_capsule
	cargo test -p unf-agent encryption_generation_
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-activation-rehydration-test: encryption-agent-anti-entropy-test
	hack/verify-encryption-activation-rehydration.sh
	cargo test -p unf-encryption recovery_plan_rehydrates
	cargo test -p unf-agent encryption_recovery_
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-local-plan-compiler-test: encryption-activation-rehydration-test
	hack/verify-encryption-local-plan-compiler.sh
	cargo test -p unf-encryption snapshot_first_compiler
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings
...
encryption-plan-runtime-test: encryption-plan-distribution-test
	hack/verify-encryption-plan-runtime.sh
	cargo test -p unf-controller encryption_plan_delivery
	cargo test -p unf-agent encryption_plan_recovery
	cargo clippy -p unf-encryption -p unf-controller -p unf-agent --all-targets --all-features -- -D warnings

encryption-plan-catalog-test: encryption-plan-runtime-test
	hack/verify-encryption-plan-catalog.sh
	cargo test -p unf-encryption fleet_synchronous
	cargo test -p unf-controller encryption_plan_delivery
	cargo clippy -p unf-encryption -p unf-controller --all-targets --all-features -- -D warnings

encryption-key-transparency-test: encryption-plan-catalog-test
	hack/verify-encryption-key-transparency.sh
	cargo test -p unf-encryption public_key_transparency
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-key-runtime-test: encryption-key-transparency-test
	hack/verify-encryption-key-runtime.sh
	cargo test -p unf-controller encryption_key_bootstrap_and_ingestion
	cargo test -p unf-agent encryption_key_bootstrap_creates
	cargo clippy -p unf-encryption -p unf-controller -p unf-agent --all-targets --all-features -- -D warnings

encryption-key-attestation-test: encryption-key-runtime-test
	hack/verify-encryption-key-attestation.sh
	cargo test -p unf-encryption reciprocal_witness
	cargo test -p unf-controller encryption_key_attestation
	cargo test -p unf-agent reciprocal_key_attestation
	cargo clippy -p unf-encryption -p unf-controller -p unf-agent --all-targets --all-features -- -D warnings

encryption-fleet-plan-producer-test: encryption-key-attestation-test
	hack/verify-encryption-fleet-plan-producer.sh
	cargo test -p unf-encryption demand_sparse_plan_cut
	cargo test -p unf-encryption fleet_plan_production_refuses
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-policy-quotient-test: encryption-fleet-plan-producer-test
	hack/verify-encryption-policy-quotient.sh
	cargo test -p unf-encryption policy_truth_quotient
	cargo test -p unf-encryption no_applicable_policy
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-address-binding-test: encryption-policy-quotient-test
	hack/verify-encryption-address-binding.sh
	cargo test -p unf-encryption replicated_identity_late_binds
	cargo test -p unf-encryption address_exact
	cargo test -p unf-agent fixed_width_encoding_matches_the_shared_encryption_abi
	cargo clippy -p unf-encryption -p unf-agent -p unf-ebpf-common --all-targets --all-features -- -D warnings

encryption-kubernetes-projection-test: encryption-address-binding-test
	hack/verify-encryption-kubernetes-projection.sh
	cargo test -p unf-encryption kubernetes_projection
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-controller-plan-test: encryption-kubernetes-projection-test
	hack/verify-encryption-controller-plan.sh
	cargo test -p unf-controller encryption_plan_
	cargo test -p unf-controller encryption_key_
	cargo clippy -p unf-encryption -p unf-controller --all-targets --all-features -- -D warnings

encryption-agent-plan-compile-test: encryption-controller-plan-test
	hack/verify-encryption-agent-plan-compile.sh
	cargo test -p unf-encryption authority_free_dormant
	cargo test -p unf-agent dormant_plan_compiles_once
	cargo test -p unf-encryption snapshot_first_compiler
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-tc-consumer-test: encryption-agent-plan-compile-test
	hack/verify-encryption-tc-consumer.sh

encryption-composition-test: encryption-tc-consumer-test
	hack/verify-encryption-composition.sh

encryption-ciphertext-live-test: encryption-composition-test
	hack/verify-encryption-ciphertext-live.sh

encryption-path-proof-test: encryption-ciphertext-live-test
	hack/verify-encryption-path-proof.sh
	cargo test -p unf-encryption path_proof
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-path-runtime-test: encryption-path-proof-test
	hack/verify-encryption-path-runtime.sh
	cargo test -p unf-controller encryption_path_
	cargo clippy -p unf-encryption -p unf-controller --all-targets --all-features -- -D warnings

encryption-path-activation-test: encryption-path-runtime-test
	hack/verify-encryption-path-activation.sh
	cargo test -p unf-encryption causal_proof_ladder
	cargo test -p unf-agent encryption_generation_
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-proof-beacon-test: encryption-path-activation-test
	hack/verify-encryption-proof-beacon.sh
	cargo test -p unf-ipam proof_beacons
	cargo test -p unf-encryption proof_beacons
	hack/verify-encryption-kernel-live.sh
	cargo clippy -p unf-ipam -p unf-encryption --all-targets --all-features -- -D warnings

encryption-path-executor-test: encryption-proof-beacon-test
	hack/verify-encryption-path-executor.sh
	cargo test -p unf-encryption path_probe
	cargo test -p unf-encryption path_proof
	cargo test -p unf-agent encryption_
	cargo clippy -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-path-live-test: encryption-path-executor-test
	hack/verify-encryption-path-executor-live.sh
	hack/verify-encryption-kernel-live.sh
	cargo test -p unf-ipam
	cargo test -p unf-encryption kubernetes_projection
	cargo clippy -p unf-ipam -p unf-encryption -p unf-agent --all-targets --all-features -- -D warnings

encryption-operations-evidence-test: encryption-path-live-test
	hack/verify-encryption-operations-evidence.sh
	cargo test -p unf-encryption operations
	cargo clippy -p unf-encryption --all-targets --all-features -- -D warnings

encryption-operations-runtime-test: encryption-operations-evidence-test
	hack/verify-encryption-operations-runtime.sh
	cargo test -p unf-controller encryption_operations
	cargo test -p unf-controller encryption_path_exchange
	cargo clippy -p unf-controller --all-targets --all-features -- -D warnings

encryption-operations-recovery-test: encryption-operations-runtime-test
	hack/verify-encryption-operations-recovery.sh
	cargo test -p unf-controller encryption_operations
...
encryption-activation-report-test: encryption-operations-recovery-test
	hack/verify-encryption-activation-report.sh
	cargo test -p unf-encryption operations
	cargo test -p unf-agent encryption_activation_outbox
	cargo test -p unf-controller encryption_operations
	cargo clippy -p unf-encryption -p unf-controller -p unf-agent --all-targets --all-features -- -D warnings

encryption-operations-query-test: encryption-activation-report-test
	hack/verify-encryption-operations-query.sh
	cargo test -p unf-controller encryption_explanation
	cargo test -p unfctl encryption_operations
	cargo clippy -p unf-controller -p unfctl --all-targets --all-features -- -D warnings

encryption-adjacent-compatibility-test: encryption-operations-query-test
	hack/verify-encryption-adjacent-compatibility.sh
	cargo test -p unf-state component_compatibility
	cargo test -p unf-agent controller_preflight_accepts_the_bounded_service_schema_transition
	cargo test -p unf-agent encryption_compatibility
	cargo test -p unf-controller component_version_exposes_the_controller_compatibility_tuple
	cargo clippy -p unf-state -p unf-encryption -p unf-agent -p unf-controller --all-targets --all-features -- -D warnings

encryption-recovery-cleanup-test: encryption-adjacent-compatibility-test
	hack/verify-encryption-recovery-cleanup.sh
	cargo test -p unf-agent encryption_cleanup
	cargo test -p unf-agent dormant_plan_compiles_once
	cargo test -p unf-agent encryption_key_bootstrap_creates
	cargo test -p unf-agent encryption_activation_outbox
	cargo test -p unf-controller encryption_operations
...
egress-native-reachability-test: egress-reachability-lifecycle-test service-kind-load
	hack/verify-egress-native-reachability.sh
	bash -n hack/verify-kind-egress-native-reachability.sh
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) kubectl --context $(SERVICE_KUBE_CONTEXT) apply -k deploy/kind-service-fabric
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) kubectl --context $(SERVICE_KUBE_CONTEXT) rollout restart deployment/unf-controller -n unf-system
...
encryption-phase9-kind-gate-test:
	hack/verify-encryption-phase9-kind-gate.sh

encryption-phase9-kind-deploy: service-kind-load
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) KUBE_CONTEXT=$(SERVICE_KUBE_CONTEXT) KIND_PROVIDER=$(KIND_PROVIDER) hack/configure-kind-primary-cni.sh
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) KUBE_CONTEXT=$(SERVICE_KUBE_CONTEXT) UNF_INTERNAL_TLS_DIR=$(CURDIR)/.tools/kind-service-internal-tls hack/configure-internal-tls.sh
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) kubectl --context $(SERVICE_KUBE_CONTEXT) apply -k deploy/kind-encryption-phase9
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) KUBE_CONTEXT=$(SERVICE_KUBE_CONTEXT) hack/configure-kind-service-bootstrap.sh
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) kubectl --context $(SERVICE_KUBE_CONTEXT) rollout restart deployment/unf-controller daemonset/unf-agent -n unf-system
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) kubectl --context $(SERVICE_KUBE_CONTEXT) rollout status deployment/unf-controller -n unf-system --timeout=180s
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) kubectl --context $(SERVICE_KUBE_CONTEXT) rollout status daemonset/unf-agent -n unf-system --timeout=180s

encryption-phase9-kind-test: encryption-phase9-kind-gate-test
	KUBECONFIG=$(SERVICE_KIND_KUBECONFIG) KUBE_CONTEXT=$(SERVICE_KUBE_CONTEXT) KIND_PROVIDER=$(KIND_PROVIDER) UNF_TEST_TOOLS_IMAGE=$(TEST_TOOLS_IMAGE) UNF_PHASE9_RUNTIME_REVISION=$(UNF_BUILD_REVISION) hack/verify-kind-encryption-phase9.sh
...
encryption-phase9-openshift-deploy:
	...
	UNF_OPENSHIFT_SERVICE_DEPLOY_EVIDENCE=$(CURDIR)/.artifacts/phase9-encryption-openshift-deploy.json \
	UNF_OPENSHIFT_SERVICE_RENDER_PATH=$(CURDIR)/deploy/openshift-primary-cni/encryption-phase9 \
	hack/deploy-openshift-service-fabric.sh

encryption-phase9-openshift-test: encryption-phase9-openshift-gate-test
	KUBECONFIG=$(OPENSHIFT_KUBECONFIG) hack/verify-openshift-encryption-phase9.sh
